...
>Aplica-se a: Windows Server 2016 Essentials, Windows Server 2012 R2 Essentials, Windows Server 2012 Essentials
 ...
 Você pode conectar os computadores ao servidor do Windows Server Essentials usando o software Connector. O software Connector é instalado quando você conectar um computador ao servidor usando o Conector usando Connecte um computador ao Assistente do servidor. Você pode iniciar esse assistente, digitando **http://<servername\>/Connect**, onde **< servername\>**  é o nome do seu servidor.  
...
##  <a name="BKMK_A"></a> Preparar para conectar computadores ao servidor  
...
-   [Informações de nome e a senha de usuário de rede](Get-Connected-in-Windows-Server-Essentials.md#BKMK_6)  
...
###  <a name="BKMK_2"></a> Pré-requisitos para conectar um computador ao servidor  
...
-   O computador cliente deve executar um sistema operacional compatível. Para obter mais informações, consulte [Supported operating systems for client computers](Get-Connected-in-Windows-Server-Essentials.md#BKMK_4).
...
-   As configurações de hora e data do computador são alinhadas com as configurações no servidor.  
...
>  Em uma implantação de cliente local para o Windows Server Essentials ou Windows Server Essentials, você pode conectar computadores ao servidor sem adicioná-los para o domínio do Windows Server Essentials. Este método não está disponível para todos os sistemas operacionais cliente com suporte e recursos, como a Política de Grupo e redes virtuais privadas (VPNs), que exige que um computador esteja conectado ao domínio, não estão disponíveis. Para encontrar os requisitos e instruções, consulte [Connect computers to a Windows Server Essentials server without joining the domain](Get-Connected-in-Windows-Server-Essentials.md#BKMK_10).  
  
 Para obter instruções passo a passo para conectar um computador ao servidor que executa o Windows Server Essentials, consulte [Connect computers to the server](Get-Connected-in-Windows-Server-Essentials.md#BKMK_9).  
...
###  <a name="BKMK_3"></a> Pré-requisitos para conectar um computador Mac à rede  
...
#### <a name="windows-server-essentials"></a>Windows Server Essentials  
  Esta seção se aplica a um servidor executando o Windows Server Essentials, ou para um servidor que executa o Windows Server 2012 R2 Standard ou Windows Server 2012 R2 Datacenter com a função experiência Windows Server Essentials instalada. Os seguintes sistemas operacionais de computador têm suporte:  
  
 **Sistemas operacionais Windows 7**  
  
-    O Windows 7 Home Basic SP1 (x86 e x64)  
  
-    O Windows 7 Home Premium SP1 (x86 e x64)  
  
-    O Windows 7 Professional SP1 (x86 e x64)  
...
> [!NOTE]
>  Você pode exibir a integridade e o status de backup para um computador Mac, do painel do Windows Server Essentials. No entanto, você não pode configurar o backup do computador ou iniciar um backup do painel. Além disso, não é possível usar o Acesso via Web Remoto para se conectar a um computador Mac.  
...
-    O Windows 7 Home Basic (x86 e x64)  
  
-    O Windows 7 Home Premium (x86 e x64)  
  
-    O Windows 7 Professional (x86 e x64)  
...
-   Cria atalhos na área de trabalho do computador para o Painel e Barra Inicial.  
...
-   Implanta VPN no computador cliente e fornece uma experiência de único clique se a funcionalidade VPN estiver habilitada no Windows Server Essentials, ou fornece uma experiência de conexão automática se a funcionalidade VPN estiver habilitada no Windows Server Essentials  
  

 Para obter informações sobre como conectar seu computador ao servidor, consulte [Connect computers to the server](Get-Connected-in-Windows-Server-Essentials.md#BKMK_9).  
  
###  <a name="BKMK_6"></a> Informações de nome e a senha de usuário de rede  
...
###  <a name="BKMK_7"></a> Conta do administrador do servidor  
...
-   Monitorar as atividades diárias na rede  
 ...
 No Windows Server Essentials, Windows Server Essentials e Windows Server 2012 R2 com a função experiência Windows Server Essentials instalada, você pode atribuir o administrador de rede nível para qualquer conta de usuário de acesso. Isso concede as permissões necessárias para executar tarefas de administrador de rede. Quando o nível de acesso de administrador de rede é atribuído a um usuário, o prompt **Controle de Acesso de Usuários** é aberto para qualquer tarefa que requeira permissões de administrador.  
...
##  <a name="BKMK_B"></a> Conectar computadores ao servidor usando o software Connector  
...
-   [Desconecte o computador do ou reconecta seu computador ao servidor](Get-Connected-in-Windows-Server-Essentials.md#BKMK_14)  
...
###  <a name="BKMK_9"></a> Conectar computadores ao servidor  
 Quando você conectar um computador a um servidor que está executando o Windows Server Essentials ou Windows Server 2012 R2 com a função experiência Windows Server Essentials instalada, certifique-se de que o computador cliente tem uma conexão válida com a Internet.  
...
3.  Na barra de endereços, digite **http://<servername\>/Connect**, e pressione Enter.  
    ...
    > [!NOTE]
    >  Se o computador estiver em um local remoto fora da rede do Windows Server Essentials, para executar Connectar um computador ao Assistente do servidor, digite **http://<domainname\>/ connect** na barra de endereços de seu (de navegador da web em que < domínio\> é o nome de domínio da sua organização). Você pode obter as informações de nome de domínio do administrador da rede.  
...
7.  O Assistente de Conexão do Computador ao Servidor será exibido. Para concluir o assistente, faça o seguinte:  
    ...
    2.  Sobre como página, **Encontrar meu servidor** página, detecção automática do servidor em redes locais e selecione o servidor que você deseja se conectar. Ou, se você tiver as informações, você pode inserir manualmente o nome do servidor ou endereço de domínio.  
...
9. Depois que o computador está conectado com êxito ao servidor, os atalhos o TrayApp conector e o painel do servidor aparecem no menu Iniciar, que pode ser usado da seguinte maneira (se o computador estiver executando o Windows 8, Windows 8.1 ou Windows 10, o painel e Connector TrayApp estarão disponível na tela inicial do computador):  
  
    -   Se o computador estiver executando o Windows 8, Windows 8.1 ou Windows 10, o painel e Connector TrayApp serão pesquisáveis como um aplicativo.  
...
###  <a name="BKMK_10"></a> Conectar computadores a um servidor do Windows Server Essentials sem ingressar no domínio  
 Este tópico descreve como adicionar um computador com Windows 7, Windows 8, Windows 8.1 ou Windows 10 para uma rede do Windows Server Essentials sem ingressar o computador no domínio do Windows Server Essentials em uma implantação de cliente local. Esse método de conexão tem suporte no Windows Server Essentials e Windows Server Essentials.  
...
-   Todos os recursos que exigem que o computador seja associado ao domínio? incluindo credenciais de domínio, diretiva de grupo e redes virtuais privadas (VPNs)? não estão disponíveis.  
...
2.  Abra um prompt de comando com direitos de administrador.  
    ...
    -   No Windows 10, clique no **inicie** botão, selecione **todos os aplicativos** -> **ferramentas do sistema Windows** -> **Prompt de comando**, o Prompt de comando com o botão direito e, em seguida, clique em **executar como administrador**.  
  
    -   No Windows 8, sobre o **inicie** página, digite **comando** e, em seguida, pressione Enter. Nos resultados, clique com botão direito **Prompt de comando**e clique em **executar como administrador**.  
  
    -   No Windows 7, sobre o **começar** menu, insira **comando** na caixa de pesquisa, clique com botão direito **Prompt de comando**e, em seguida, clique em **executar como administrador**.  
...
4.  Conclua as etapas em [Connect computers to the server](Get-Connected-in-Windows-Server-Essentials.md#BKMK_9).  

  
####  <a name="BKMK_SecondServer"></a> Adicionar um segundo servidor à rede  
...
3.  Se a configuração de segurança aprimorada do Internet Explorer estiver habilitada no servidor que você está tentando se conectar à rede Windows Server Essentials, execute o seguinte; Caso contrário, ignore esta etapa.  
    ...
    2.  Adicione a **http://<servername\>/Connect** site aos sites confiáveis da seguinte maneira:  
    ...
    > [!NOTE]
    >  Conectando um segundo servidor a um servidor que executa o Windows Server Essentials é diferente de se conectar ao computador cliente da seguinte maneira:  
    >   
    >  -   Não há nenhuma migração de perfil do usuário; Portanto, o perfil atual não será migrado.  
    > -   É possível fazer backup no segundo servidor usando o backup do computador cliente, e não há a opção de ativar o computador para backup.  
...
-   Se você selecionar o segundo servidor e, em seguida, clique no **exibir as propriedades do servidor** de tarefas, não há nenhum **Backup** guia exibida na página de propriedades do servidor.  
  
-   Como não há nenhuma Central de Segurança em um sistema operacional Windows Server, o status de segurança do segundo servidor é aparece como **não aplicável**.  
  
-   Exibe o status da Política de Grupo do segundo servidor como **não aplicável**.  
  
###  <a name="BKMK_11"></a> Instalar o software Connector  
 O software Connector no Windows Server Essentials é instalado quando você conecta o computador ao servidor usando a conectar um computador para o Assistente do servidor. Você pode iniciar esse assistente digitando **http://<ServerName\>/Connect** na barra de endereços do navegador da web (em que *< ServerName\>*  é o nome do seu servidor).  
  
> [!NOTE]
>  Se o computador estiver em um local remoto, para executar Connectar um computador ao Assistente do servidor, digite **http://<domainname\>/Connect** na barra de endereços do navegador da web (onde *< domínio\>*  é o nome de domínio da sua organização). Você pode obter as informações de nome de domínio do administrador da rede.  
...
-   Permite configurar e administrar remotamente o Windows Server Essentials em seu computador doméstico  
 ...
 Para obter instruções passo a passo sobre a conexão de seu computador ao servidor Windows Server Essentials, consulte [Connect computers to the server](Get-Connected-in-Windows-Server-Essentials.md#BKMK_9).   

  
###  <a name="BKMK_12"></a> Mover dados do computador e as configurações manualmente  
  Windows Server Essentials e Windows Server Essentials dar suporte a migração de perfil do usuário apenas para computadores cliente que estão executando o sistema operacional Windows 7. Quando você conectar um computador baseado no Windows 7 para o servidor, o computador se conectar ao Assistente de servidor pode migrar automaticamente o perfil de usuário.  
  
 O perfil do usuário não pode ser transferido automaticamente ao se conectar a um Windows 8, Windows 8.1 ou o computador com Windows 10 para o servidor. No entanto, em um computador com Windows 8, você pode usar a transferência fácil do Windows para transferir dados e configurações do usuário local original para o computador associado ao domínio. Para fazer isso, você deve ser um administrador no computador de origem Windows 8 e o computador de destino do Windows 8. Para obter informações sobre como usar a Transferência Fácil do Windows para transferir arquivos e configurações, consulte o [artigo 2735227](https://support.microsoft.com/kb/2735227) na Base de Dados de Conhecimento da Microsoft.  
...
1.  Em um computador que está executando o Windows 7, Windows 8, Windows 8.1 ou Windows 10, abra **painel de controle**e, em seguida, o **programas** seção, clique em **exibir atualizações instaladas**.  
...
5.  No Windows Server Essentials, se a página do Windows Server Essentials Connector aparecer sugiram para fechar a barra inicial, clique em **Okey**.  
  
6.  Aguarde até que o programa de desinstalação. Depois que o software for removido, **Windows Server Essentials Connector** não é mais exibido na lista de programas instalados ou atualizações. Além disso, os atalhos da barra inicial e o Painel não são exibidos na área de trabalho do computador.  
  
> [!NOTE]
>  -   Desinstalar o software Connector não remover o computador da lista de computadores que são exibidos em **DISPOSITIVOS** no Painel. Para remover o computador do Painel, consulte [Remove a computer from the server](../manage/Manage-Devices-in-Windows-Server-Essentials.md#BKMK_3).  
...
> -   Desinstalar o software Connector não desassocia o computador do domínio original. Você deve manualmente desassocie o computador do domínio. Para obter instruções, consulte [Remove a computer from a Windows domain](Get-Connected-in-Windows-Server-Essentials.md#BKMK_8).  

  
###  <a name="BKMK_14"></a> Desconecte o computador do ou reconecta seu computador ao servidor  
...
3.  Remova o computador do servidor usando o Painel. Para obter instruções passo a passo, consulte [Remove a computer from the server](../manage/Manage-Devices-in-Windows-Server-Essentials.md#BKMK_3).  
