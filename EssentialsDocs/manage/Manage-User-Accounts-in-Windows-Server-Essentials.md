...
##  <a name="BKMK_ManageAccounts"></a>Gerenciando contas de usuário  
...
###  <a name="BKMK_Manage8"></a>Gerenciar contas de usuário usando o painel  
 O Windows Server Essentials torna possível executar tarefas administrativas comuns usando o Painel do Windows Server Essentials. Por padrão, a página **usuários** do painel inclui duas guias: Grupos **usuários** e **usuários**.  
...
##  <a name="BKMK_ManageAccess"></a>Gerenciando senhas e acesso  
...
- **Comprimento da senha**.  Quanto mais longa for a senha, mais segura será. Senhas em branco não são seguras.  
  
- **Complexidade de senha**.  Senhas complexas contêm uma mistura de letras maiúsculas e minúsculas (a-z, A-Z), números básicos (0-9) e símbolos não alfabéticos (como;!, @, #, _,-). Senhas complexas são muito menos suscetíveis a acesso não autorizado. As senhas que contêm nomes de usuário, datadenascimentos ou outras informações pessoais não fornecem segurança adequada.  
...
-   **Leitura/gravação**.  Escolha essa configuração se desejar permitir que a conta de usuário tenha permissão para criar, alterar e excluir todos os arquivos na pasta compartilhada.  
...
###  <a name="BKMK_Access6"></a>Reter e gerenciar o acesso a arquivos para contas de usuário removidas  
...
1. Em um prompt de comando, `ntdsutil.exe` execute para abrir a ferramenta Ntdsutil.  
...
3. Para sincronizar a senha do DSRM em um controlador de domínio com a conta do administrador de rede atual, digite:  
    ...
    **sincronizar da conta de domínio** *< current_network_administrator_account >* e pressione Enter.  
...
###  <a name="BKMK_Access8"></a>Conceder permissão de área de trabalho remota às contas de usuário  
...
###  <a name="BKMK_Access9"></a>Permitir que os usuários acessem recursos no servidor  
...
###  <a name="BKMK_Access13"></a>Permitir que contas de usuário estabeleçam uma sessão de área de trabalho remota para seu computador  
  Esta seção se aplica a um servidor que executa o Windows Server Essentials ou o Windows Server Essentials ou a um servidor executando o Windows Server 2012 R2 Standard ou o Windows Server 2012 R2 Datacenter com a função de experiência do Windows Server Essentials instalada.  
