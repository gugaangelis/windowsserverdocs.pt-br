...
-   [Permitir que os dispositivos de mídia que usam o protocolo de bloco de mensagens de servidor (SMB) para acessar arquivos compartilhados no servidor](Manage-Digital-Media-in-Windows-Server-Essentials.md#BKMK_10)  
  
-   [Processadores comuns e os perfis de vídeos que eles dão suporte a](Manage-Digital-Media-in-Windows-Server-Essentials.md#BKMK_CommonProcessors)  
...
###  <a name="BKMK_1.5"></a> Compartilhamento de mídia protegida contra cópia  
...
##  <a name="BKMK_2"></a> Gerenciar o servidor de mídia usando o painel  
  Windows Server Essentials possibilita realizar tarefas administrativas comuns usando o painel do Windows Server Essentials. O guia **mídia** do servidor **configurações** na página do Painel fornece o seguinte:  
...
##  <a name="BKMK_3"></a> Como funciona o streaming de mídia  
 A mídia de streaming de recurso no Windows Server Essentials torna possível para computadores em rede e alguns dispositivos de mídia digital em rede reproduzir arquivos de mídia digital armazenados no servidor.  
...
- Faixa de CD de áudio (. cda)  
  ...
  Para executar um arquivo, basta localizar uma música, vídeo ou imagem em uma pasta compartilhada, clicar duas vezes no arquivo e o conteúdo será transmitido do servidor para o computador e executado. Para obter informações sobre como localizar e executar arquivos de mídia digital armazenados no servidor, consulte [reproduzir mídia Digital](../use/Play-Digital-Media-in-Windows-Server-Essentials.md).  
  
  Para transmitir sua mídia, você precisa do seguinte hardware:  
  
- Uma rede privada com ou sem fio  
  
- Qualquer outro computador na sua rede ou um dispositivo conhecido como um receptor de mídia digital (às vezes chamado de um reprodutor de mídia digital em rede). Receptores de mídia digital são dispositivos de hardware conectados à sua rede com ou sem fio que você pode controlar por meio de seu computador?, mesmo se o computador estiver em outra sala.  
  
  Para obter mais informações, consulte [ativar ou desativar o streaming de mídia](Manage-Digital-Media-in-Windows-Server-Essentials.md#BKMK_4).  
  
##  <a name="BKMK_4"></a> Ativar ou desativar o streaming de mídia  
 Você pode compartilhar músicas, vídeos e imagens do Windows Server Essentials por streaming de arquivos para qualquer receptor de mídia digital compatíveis (DMR), como computadores, celulares, televisores, receptores de mídia digital, extenders do Windows Media Center (incluindo Xbox 360) e outros dispositivos eletrônicos pessoais.  
  
 Para obter uma lista atual de dispositivos de mídia digital que são compatíveis com o Windows Server Essentials, consulte a [Centro de compatibilidade do Windows](https://www.microsoft.com/windows/compatibility/CompatCenter/Home).  
...
##  <a name="BKMK_5"></a> Adicionar arquivos de mídia digital para o servidor  
...
1.  Use um dos métodos a seguir para entrar no servidor:  
    ...
    2.  Para obter informações sobre como entrar com barra inicial, consulte [visão geral da barra inicial](Overview-of-the-Launchpad-in-Windows-Server-Essentials.md).  
...
-   Quando você ativa o compartilhamento de mídia, ele cria quatro pastas predefinidas: Música, imagens, vídeos e TV gravada. Se houver qualquer uma das pastas no servidor, a pasta existente será reutilizada como uma pasta compartilhada para compartilhamento de mídia. Permissões de usuário e conteúdo de mídia de todos os existentes da pasta são preservadas, e elas são compartilhadas com todos os usuários da rede.  
...
-   Se você ativar o compartilhamento de biblioteca de mídia para uma pasta compartilhada, qualquer reprodutor de mídia digital compatível ou DMR que puder acessar sua rede do Windows Server Essentials também pode acessar a mídia digital na pasta compartilhada. Por exemplo, se você tiver uma rede sem fio e ela não estiver protegida, qualquer pessoa dentro do alcance de sua rede sem fio potencialmente pode acessar sua mídia digital nessa pasta. Antes de ativar o compartilhamento de biblioteca de mídia, certifique-se de proteger sua rede sem fio. Para obter mais informações, veja a documentação do ponto de acesso sem fio.  
  
##  <a name="BKMK_8"></a> Renomear a biblioteca de mídia  
 O nome padrão da biblioteca de mídia é **servidor de mídia Digital**. Ele é criado quando você ativa o streaming de mídia no Windows Server Essentials. Para obter mais informações sobre ativação streaming de mídia, consulte [para ativar ou desativar o streaming de mídia](Manage-Digital-Media-in-Windows-Server-Essentials.md#BKMK_2.5). Você pode modificar o nome da biblioteca de mídia a qualquer momento usando o painel do servidor.  
...
##  <a name="BKMK_9"></a> Interromper o compartilhamento de mídia digital  
 O administrador do servidor pode interromper o compartilhamento de mídia digital que é armazenadas em pastas compartilhadas em um servidor que executa o Windows Server Essentials.  
...
##  <a name="BKMK_10"></a> Permitir que os dispositivos de mídia que usam o protocolo de bloco de mensagens de servidor (SMB) para acessar arquivos compartilhados no servidor  
...
##  <a name="BKMK_CommonProcessors"></a> Processadores comuns e os perfis de vídeos que eles dão suporte a  
 Para transmitir a mídia do seu servidor do Windows Server Essentials, você pode usar um computador que está executando o sistema operacional Windows 7 ou Windows 8 ou outros dispositivos de rede (como players de mídia digital) ou Media Center extenders (como Xbox 360). Quando você estiver usando sua rede, use remoto da Web Access Media Player para executar arquivos que são armazenados no servidor.  
 ...
 Você precisa de uma taxa de transferência de dados entre 200 KBps e 10 MBps. Você precisa usar formatos de mídia que seu computador e dispositivos podem reconhecer e reproduzir. Nem todos os dispositivos oferecem suporte a formatos de mídia, deve haver uma forma de seu computador e dispositivos para executar os arquivos de mídia que você tem.  
  ...
  Windows Server Essentials contém suporte a transcodificação que determina a capacidade do computador ou dispositivo e converte dinamicamente um arquivo de vídeo não compatível em compatível. Em geral, se o Windows Media Player 12 pode executar o conteúdo em um computador que esteja executando o Windows 7 ou Windows 8, o conteúdo no servidor funcionará no dispositivo conectado à rede.  
...
- Em um computador de rede que executam o Windows 7 ou Windows 8 que tem o mesmo processador do seu servidor, vá para o **painel de controle**, clique em **informações de desempenho e ferramentas**e, em seguida, examine as informações sobre o **Taxa e melhorar o desempenho do seu computador** página.  
