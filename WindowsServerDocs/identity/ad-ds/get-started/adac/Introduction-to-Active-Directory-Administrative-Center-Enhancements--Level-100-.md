...
- [Visualizador de histórico do PowerShell do Windows](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#windows_powershell_history_viewer)

## <a name="ad_recycle_bin_mgmt"></a>Lixeira do Active Directory

A exclusão acidental de objetos do Active Directory é uma ocorrência comum aos usuários do AD DS (Serviços de Domínio Active Directory) e do AD LDS (Active Directory Lightweight Directory Services). Em versões anteriores do Windows Server, anteriores ao Windows Server 2008 R2, era possível recuperar objetos excluídos acidentalmente do Active Directory, mas as soluções apresentavam algumas desvantagens.
...
Se você planeja habilitar a Lixeira do Active Directory no Windows Server, considere o seguinte:

- Por padrão, a Lixeira do Active Directory é desabilitada. Para habilitá-lo, primeiro você deve aumentar o nível funcional da floresta do ambiente do AD DS ou do AD LDS para o Windows Server 2008 R2 ou superior. Isso por sua vez requer que todos os controladores de domínio na floresta ou todos os servidores que hospedem instâncias de conjuntos de configuração do AD LDS estejam executando o Windows Server 2008 R2 ou superior.
...
- Para gerenciar o recurso Lixeira via interface do usuário, você deve instalar a versão da Central Administrativa do Active Directory no Windows Server 2012.

    > [!NOTE]
    > Você pode usar **Gerenciador do servidor** para instalar o Remote Server Administration RSAT (ferramentas) para usar a versão correta da Central Administrativa do Active Directory para gerenciar a Lixeira via interface do usuário.
    >
...
As etapas a seguir, você usará o ADAC para executar as seguintes tarefas da Lixeira do Active Directory no Windows Server 2012:

- [Etapa 1: Aumentar o nível funcional de floresta](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#bkmk_raise_ffl)
...
- [Etapa 3: Criar usuários de teste, o grupo e a unidade organizacional](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#bkmk_create_test_env)
...
Nesta etapa, você aumentará o nível funcional da floresta. Primeiro, você deve aumentar o nível funcional de floresta de destino para ser o Windows Server 2008 R2 no mínimo antes de habilitar a Lixeira do Active Directory.

#### <a name="to-raise-the-functional-level-on-the-target-forest"></a>Para aumentar o nível funcional na floresta de destino

1. Clique com botão direito no ícone do Windows PowerShell, clique em **executar como administrador** e digite **dsac.exe** para abrir o ADAC.
...
3. Clique no domínio de destino no painel de navegação esquerdo e, no painel **Tarefas**, clique em **Aumentar nível funcional da floresta**. Selecione um nível funcional de floresta que tenha pelo menos Windows Server 2008 R2 ou superior e, em seguida, clique em **Okey**.

![Introdução ao centro de administração do AD](media/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-/PowerShellLogoSmall.gif)***<em>comandos equivalentes do Windows PowerShell</em>***
...
Para o **-identidade** argumento, especifique o nome de domínio DNS totalmente qualificado.
...
1. Clique com botão direito no ícone do Windows PowerShell, clique em **executar como administrador** e digite **dsac.exe** para abrir o ADAC.
2. Clique em **Manage**, clique em **adicionar nós de navegação** e selecione o domínio de destino apropriado na **adicionar nós de navegação** caixa de diálogo e depois clique em * * Okey
...
No Windows Server 2012 e mais recente de senha refinadas gerenciamento de política é feito mais fácil e mais visual, fornecendo uma interface do usuário para administradores do AD DS para gerenciá-las no ADAC. Os administradores agora podem exibir a política resultante de um determinado usuário, exibir e classificar todas as políticas de senha em um domínio específico e gerenciar políticas de senha individuais visualmente.

Se você planejar usar políticas de senha refinada no Windows Server 2012, considere o seguinte:
...
- Você deve usar o Windows Server 2012 ou versão mais recente da Central Administrativa do Active Directory para administrar políticas de senha refinada por meio de uma interface gráfica do usuário.
...
- [Etapa 4: Exibir um conjunto resultante de diretivas para um usuário](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#bkmk_view_resultant_fgpp)
...
O procedimento a seguir, você aumentará o nível funcional de domínio do domínio de destino para o Windows Server 2008 ou superior. Um nível funcional de domínio do Windows Server 2008 ou superior é necessário para habilitar políticas de senha refinada.
...
3. Clique no domínio de destino no painel de navegação esquerdo e, no **Painel de Tarefas**, clique em **Aumentar nível funcional do domínio**. Selecione um nível funcional de floresta que tenha pelo menos Windows Server 2008 ou superior e depois clique em **Okey**.
...
Para criar os usuários de teste e grupo necessários para esta etapa, siga os procedimentos localizados aqui: [Etapa 3: Criar usuários de teste, o grupo e a unidade organizacional](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#bkmk_create_test_env) (não é necessário criar a unidade Organizacional para demonstrar a política de senha refinada).
...
No procedimento a seguir, você exibirá as configurações de senha resultantes para um usuário que seja membro do grupo ao qual você atribuiu uma política de senha refinada em [etapa 3: Criar uma nova política de senha refinada](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#bkmk_create_fgpp).
...
3. Selecione um usuário, **test1** que pertence ao grupo de **group1** que você associou a uma política de senha refinada em [etapa 3: Criar uma nova política de senha refinada](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#bkmk_create_fgpp).
...
No procedimento a seguir, você editará a política de senha refinada que você criou no [etapa 3: Criar uma nova política de senha refinada](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#bkmk_create_fgpp)
...
4. Selecione a política de senha refinada que você criou no [etapa 3: Criar uma nova política de senha refinada](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#bkmk_create_fgpp) e clique em **Properties** no **tarefas** painel.
...
4. Selecione a política de senha refinada que você criou no [etapa 3: Criar uma nova política de senha refinada](../../../ad-ds/get-started/adac/Introduction-to-Active-Directory-Administrative-Center-Enhancements--Level-100-.md#bkmk_create_fgpp) e, em de **tarefas** painel clique **propriedades**.
...
- Ter uma compreensão básica do Windows PowerShell. Por exemplo, é preciso saber como funciona o pipe no Windows PowerShell. Para obter mais informações sobre pipe no Windows PowerShell, consulte [Pipe e pipeline no Windows PowerShell](https://technet.microsoft.com/library/ee176927.aspx).
...
[Avançadas de gerenciamento do AD DS usando o Centro Administrativo do Active Directory &#40;nível 200&#41;](Advanced-AD-DS-Management-Using-Active-Directory-Administrative-Center--Level-200-.md)
