...
|Fase|Log|  
|---------|-------|  
|Gerenciador do Servidor ou operações ADDSDeployment do Windows PowerShell|-   %systemroot%\debug\dcpromoui.log<br /><br />-   %systemroot%\debug\dcpromoui*.log|  
|Instalação/Promoção do controlador de domínio|-   %systemroot%\debug\dcpromo.log<br /><br />-   %systemroot%\debug\dcpromo*.log<br /><br />-Eventos Visualizador logs\system.<br /><br />-Log\aplicativos do Visualizador de eventos<br /><br />-Event viewer\Applications and services \ serviço de diretório<br /><br />-Event viewer\Applications and services logs\File serviço de replicação<br /><br />-Event viewer\Applications and services \ replicação do DFS|  
|Atualização de floresta ou domínio|-   %systemroot%\debug\adprep\\<datetime>\adprep.log<br /><br />-   %systemroot%\debug\adprep\\<datetime>\csv.log<br /><br />-   %systemroot%\debug\adprep\\<datetime>\dspecup.log<br /><br />-   %systemroot%\debug\adprep\\<datetime>\ldif.log*|  
|Mecanismo de implantação de ADDSDeployment do Windows PowerShell no Gerenciador do Servidor|-Event viewer\Applications and services Deployment\Operational logs\Microsoft\Windows\DirectoryServices|  
|Serviço do Windows|-   %systemroot%\Logs\CBS\\*<br /><br />-   %systemroot%\servicing\sessions\sessions.xml<br /><br />-   %systemroot%\winsxs\poqexec.log<br /><br />-   %systemroot%\winsxs\pending.xml|  
...
-   [Network Monitor 3.4](https://www.microsoft.com/download/en/details.aspx?displaylang=en&id=4865) (ou uma ferramenta de captura e análise de rede terceiros)  
...
3.  É erro na promoção e, portanto, fatal?  
    ...
    2.  Examine o Dcpromoui.log e o dcpromo.log quanto aos erros mostrados na saída, depois trabalhe neles na ordem de chegada para ver indicações do motivo para a ocorrência da falha.  
        ...
        1.  Compare sempre com uma amostra de log do trabalho  
...
## <a name="troubleshooting-events-and-error-messages"></a>Solução de problemas de eventos e mensagens de erro
...
### <a name="promotion-and-demotion-failure-codes"></a>Códigos de promoção e rebaixamento de falha
...
|     12     |                                                    Usuário deve ser administrador                                                    |                                                                                        Faça logon como um membro do grupo Administradores interno e certifique-se de estar sendo elevado com UAC                                                                                        |
...
|     21     |                             Este controlador de domínio detém funções FSMO, é um GC e/ou é um servidor DNS                             |                                                                                                      Adicione **- demoteoperationmasterrole** ao usar **- forceremoval**.                                                                                                      |
...
|     50     | Durante o rebaixamento, o último controlador de domínio foi detectado, embora não esteja, ou o último controlador de domínio foi especificado, mas não está. |        Não especifique **Último Controlador de Domínio no Domínio** ( **-lastdomaincontrollerindomain**), a menos que seja verdadeiro. Use **- ignorelastdcindomainmismatch** substituir se essa é realmente o último controlador de domínio e não há metadados do controlador de domínio fantasma        |
...
|     52     |            O argumento de linha de comando necessário está ausente (isto é, um arquivo de resposta deve ser especificado na linha de comando)             |                                                                                              *Visto apenas com dcpromo /unattend, que está obsoleto. Consulte a documentação mais antiga*                                                                                              |
...
|     61     |        Não é possível instalar os Serviços de Domínio Active Directory com DNS em um domínio existente que já não hospede DNS         |                                                                                                                      *Não é possível receber esse erro*                                                                                                                      |
|     62     |                                         O arquivo de resposta não tem uma seção [DCInstall]                                          |                                                                                             *Visto apenas com dcpromo /unattend, que está obsoleto. Consulte a documentação mais antiga.*                                                                                              |
...
|     68     |                                                  Parceiro de replicação inválido                                                  |                                                                             Use repadmin.exe ou o **Get-ADReplication\\**  \* Windows PowerShell para validar a integridade do controlador de domínio parceiro                                                                              |
...
|     70     |                                          O controlador de domínio raiz da floresta deve ser um GC                                          |                                                                                              *Visto apenas com dcpromo /unattend, que está obsoleto. Consulte a documentação mais antiga*                                                                                              |
...
|     91     |                                             Falha ao detectar se o processo é wow64                                             |                                                                                                  *Não é possível receber esse erro, o sistema operacional é 64 bits*                                                                                                  |
|     92     |                                                  O processo Wow64 não é compatível                                                  |                                                                                                  *Não é possível receber esse erro, o sistema operacional é 64 bits*                                                                                                  |
...
|     96     |                                                 Não é possível desinstalar binários DS                                                  |                                                                                              *Visto apenas com dcpromo /unattend, que está obsoleto. Consulte a documentação mais antiga*                                                                                              |
...
|     98     |                                        A instalação/desinstalação dos arquivos binários do componente está em andamento.                                        |                                                                                              *Visto apenas com dcpromo /unattend, que está obsoleto. Consulte a documentação mais antiga*                                                                                              |
...
## <a name="known-issues-and-common-support-scenarios"></a>Problemas conhecidos e cenários comuns de suporte
...
|Solução e notas|Ao remover a função AD DS, remova também a função Servidor DNS ou defina o serviço Servidor DNS como desabilitado. Lembre-se de apontar o cliente DNS para um servidor diferente. Se estiver usando o Windows PowerShell, execute o seguinte depois de rebaixar o servidor:<br /><br />Código - dns-windowsfeature desinstalar<br /><br />ou<br /><br />Código - dns - starttype do conjunto de serviço desabilitado<br />Parar serviço dns|  
...
|---------|------------------------------------------------------------------|  
|Sintomas|Erro retornado:<br /><br />Código – não foi possível validar o caminho de mídia. Exceção ao chamar "GetDatabaseInfo" argumentos "2". A pasta não é válida.|  
...
|---------|-------------------------------------------------------------------------|  
|Sintomas|Aviso retornado *duas vezes* ao promover usando o ADDSDeployment do Windows PowerShell:<br /><br />Código – "uma delegação para esse servidor DNS não pode ser criada porque a zona pai autoritativa não pode ser encontrada ou não executa Windows DNS server. Se você estiver integrando uma infraestrutura DNS existente, você deve criar manualmente uma delegação para esse servidor DNS na zona pai para assegurar resolução de nome confiável de fora do domínio. Caso contrário, nenhuma ação é necessária."|  
...
|---------|--------------------------------------------------------------------------------------------|  
|Sintomas|O Gerenciador do Servidor retorna o erro:<br /><br />Código - exceção chamando "DNSOption" argumentos "6"<br /><br />O ADDSDeployment do Windows PowerShell retorna erro:<br /><br />Falha no código - verificação de permissões de usuário. Você deve fornecer o nome do domínio ao qual pertence essa conta de usuário.|  
...
|---------|---------------------------------------------------------------------------------------------------|  
|Sintomas|Se estiver usando Dism.exe para remover a função AD DS antes de rebaixar sem problemas um controlador de domínio, o servidor já não será iniciado mais normalmente e mostrará erro:<br /><br />Código - Status: 0x000000000<br />INFO: Ocorreu um erro inesperado.|  
...
|---------|--------------------------------------------------------------------|  
|Sintomas|A promoção usando o ADDSDeployment do Windows PowerShell retorna o erro:<br /><br />Código - Test.VerifyDcPromoCore.DCPromo.General.74<br /><br />Falha na verificação de pré-requisitos para a promoção do controlador de domínio. O nível funcional do domínio especificado é inválido|  
|Solução e notas|Não especifique um modo funcional de floresta do Win2012 sem especificar *também* um modo funcional de domínio do Win2012. Aqui está um exemplo que irá funcionar sem erros:<br /><br />Code - -forestmode Win2012 -domainmode Win2012]|  
...
|Solução e notas|Essa é uma limitação do Gerenciador do Servidor. Para comentários, use o cmdlet ADDSDeployment do Windows PowerShell:<br /><br />Código - addsdomaincontroller desinstalar|  
...
|Sintomas|Ao promover um novo controlador de domínio usando IFM e fornecendo uma mídia incorreta ao IFM - tal como a mídia RODC para um controlador de domínio gravável ou a mídia RWDC para um RODC - o botão Verificar não retorna um erro. Mais tarde, a promoção falha com o erro:<br /><br />Código - ocorreu um erro ao tentar configurar este computador como um controlador de domínio. <br />A promoção de mídia de instalação de um controlador de domínio somente leitura não pode iniciar porque o banco de dados de origem especificado não é permitido. Somente bancos de dados de outros RODCs podem ser usados para a promoção IFM de um RODC.|  
...
|Sintomas|Se selecionar (ou não) a opção de Gerenciador de servidores **reiniciar cada servidor de destino automaticamente se necessário** whendemoting um controlador de domínio por meio de remoção de função, o servidor sempre reinicia, independentemente da escolha.|  
...
|Sintomas|O rebaixamento de um controlador de domínio é concluído sem problemas, mas o exame do log dcpromo mostra erro:<br /><br />Código - [error] Configurando a segurança em arquivos do servidor falhou com 2|  
...
|Sintomas|Ao tentar promover um controlador de domínio do Windows Server 2012 para uma floresta existente do Windows Server 2003, Windows Server 2008 ou Windows Server 2008 R2, a verificação dos pré-requisitos falha com o erro:<br /><br />Falha no código - verificação de pré-requisitos para a preparação do AD. Não é possível executar a verificação de conflitos de esquema do Exchange para o domínio *<domain name>* (exceção: o servidor RPC não está disponível)<br /><br />O adprep.log mostra o erro:<br /><br />Código - Adprep não foi possível recuperar dados do servidor *<domain controller>*<br /><br />por meio da instrumentação de gerenciamento do Windows (WMI).|  
|Solução e notas|O novo controlador de domínio não pode acessar a WMI através de protocolos DCOM/RPC entre os controladores de domínio existentes. Até o momento, houve três causas para isso:<br /><br />-Um firewall regra bloqueia o acesso a controladores de domínio existentes<br /><br />-A conta de serviço de rede está ausente de "Logon como um serviço" (SeServiceLogonRight) privilégio nos controladores de domínio existentes<br /><br />-NTLM está desabilitado nos controladores de domínio, usando as políticas de segurança descritas em [apresentando a restrição da autenticação NTLM](https://technet.microsoft.com/library/dd560653(WS.10).aspx)|  

|||  
|-|-|  
|Problema|A criação de uma nova floresta AD DS sempre mostra um aviso de DNS|  
|Sintomas|Ao criar uma nova floresta do AD DS e a zona DNS no novo controlador de domínio para ele mesmo, você sempre recebe uma mensagem de aviso:<br /><br />Código – um erro foi detectado na configuração de DNS. <br />Nenhum dos servidores DNS usados por este computador respondeu dentro do intervalo de tempo limite.<br />(código de erro 0x000005B4 "ERROR_TIMEOUT")|  
...
|Sintomas|Depois de promover um novo controlador de domínio e, em seguida, fazer logoff e tentar fazer logon interativamente, você recebe um erro:<br /><br />Código – não há armazenamento suficiente está disponível para processar este comando|  
...
|Solução e notas|Você tem vários sites do AD DS e, em pelo menos um, as sub-redes estão ausentes; esse futuro controlador de domínio pertence a uma dessas sub-redes. Você deve selecionar manualmente a sub-rede no menu suspenso Nome do site. Você também deve verificar todos os sites de AD usando DSSITE.MSC ou use o seguinte comando do Windows PowerShell para encontrar todas as sub-redes que faltam nos sites:<br /><br />Código - get-adreplicationsite-filtro \* -subredes a propriedade &#124; where-object {! $_.subnets - eq "\*"} &#124; nome da tabela de formato|  

|||  
|-|-|  
|Problema|A promoção ou rebaixamento falha com a mensagem "o serviço não pode ser iniciado"|  
|Sintomas|Se você tentar a promoção, o rebaixamento ou a clonagem de um controlador de domínio, receberá um erro:<br /><br />Código - o serviço não pode ser iniciado, porque ele é desabilitado ou ele tem não há dispositivos habilitados associados a ele"(0x80070422)<br /><br />O erro pode ser interativo, um evento ou gravado em um log como dcpromoui.log ou dcpromo.log|  
...
|-|-|  
|Sintomas|Se você promover um controlador de domínio usando dcpromo /unattend com o seguinte arquivo de resposta de amostra:<br /><br />Código-<br /><br />[DCInstall]<br />NewDomain=Forest<br /><br />ReplicaOrNewDomain=Domain<br /><br />NewDomainDNSName=corp.contoso.com<br /><br />SafeModeAdminPassword=Safepassword@6<br /><br />DomainNetbiosName=corp<br /><br />DNSOnNetwork=Yes<br /><br />AutoConfigDNS=Yes<br /><br />RebootOnSuccess=NoAndNoPromptEither<br /><br />RebootOnCompletion=No<br /><br />*DomainLevel=0*<br /><br />*ForestLevel=0*<br /><br />A promoção falha com os seguintes erros no dcpromoui.log:<br /><br />Código - dcpromoui 0089 EA4.5B8 13:31:50.783 Enter CArgumentsSpec::ValidateArgument DomainLevel<br /><br />Dcpromoui EA4.5B8 008A 13:31:50.783 valor para DomainLevel é 0<br /><br />Dcpromoui EA4.5B8 008B 13:31:50.783 código de saída é 77<br /><br />Dcpromoui EA4.5B8 008C 13:31:50.783 o argumento especificado é inválido.<br /><br />log de fechamento de 13:31:50.783 D 008 EA4.5B8 Dcpromoui<br /><br />Dcpromoui 0032 EA4.5B8 13:31:50.830 código de saída é 77<br /><br />O nível 0 é o Windows 2000, que não é compatível com o Windows Server 2012.|  
...
|Problema|Promoção "trava" ao criar o objeto de configurações NTDS, nunca é concluído|  
|-|-|  
|Sintomas|Se você promover uma réplica de controlador de domínio ou RODC, a promoção atinge "Criando objeto de configurações NTDS" e nunca prossegue ou conclui. Os logs param de atualizar também.|  
|Solução e notas|Este é um problema conhecido causado pelo fornecimento de credenciais da conta interna de Administrador local com uma senha correspondente à conta interna de Administrador do domínio. Isso causa uma falha no mecanismo da instalação principal que não incorre em erro mas, em vez disso, aguarda indefinidamente (quasi-loop). Isso é esperado – embora indesejável - comportamento.<br /><br />Para corrigir o servidor:<br /><br />1.  Reinicialize-o.<br /><br />1.  No AD, exclua a conta do computador membro desse servidor (ele não ainda será uma conta de controlador de domínio)<br /><br />1.  No servidor, force sua separação do domínio<br /><br />1.  Nesse servidor, remova a função AD DS.<br /><br />1.  Reinicialização<br /><br />1.  Adicione novamente o AD DS função e tente promoção, certificando-se de sempre fornecer os ***domain\admin*** formatado credenciais para a promoção do controlador de domínio e não apenas a conta de administrador local interna|  
