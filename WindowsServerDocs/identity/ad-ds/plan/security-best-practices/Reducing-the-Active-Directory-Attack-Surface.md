...
-   A [implementação de modelos administrativos de privilégios mínimos](../../../ad-ds/plan/security-best-practices/../../../ad-ds/plan/security-best-practices/Implementing-Least-Privilege-Administrative-Models.md) se concentra na identificação do risco de que o uso de contas altamente privilegiadas para a administração diária apresenta, além de fornecer recomendações para implementar a fim de reduzir o risco que contas com privilégios presentes.  
...
> [!NOTE]  
> Um atributo no objeto AdminSDHolder, dSHeuristics, permite a personalização limitada (remoção) de grupos que são considerados grupos protegidos e são afetados pelo AdminSDHolder e pelo SDProp. Essa personalização deve ser considerada cuidadosamente se for implementada, embora haja circunstâncias válidas nas quais a modificação do dSHeuristics em AdminSDHolder seja útil. Mais informações sobre a modificação do atributo dSHeuristics em um objeto AdminSDHolder podem ser encontradas nos artigos de Suporte da Microsoft [817433](https://support.microsoft.com/?id=817433) e [973840](https://support.microsoft.com/kb/973840)e em [Appendix C: Contas e grupos protegidos no Active Directory @ no__t-0.  
  
Embora os grupos mais privilegiados no Active Directory sejam descritos aqui, há vários outros grupos que receberam níveis elevados de privilégio. Para obter mais informações sobre todos os grupos padrão e internos no Active Directory e os direitos de usuário atribuídos a cada um, consulte [Appendix B: Contas e grupos com privilégios no Active Directory @ no__t-0.  
