...
- Incompatibilidade de função do mestre de operações de infraestrutura: Não coloque o catálogo global em um controlador de domínio que hospede a função mestre de operações de infraestrutura no domínio, a menos que todos os controladores de domínio no domínio sejam servidores de catálogo global ou a floresta tenha apenas um domínio.  
