---
title: Por que as estações de trabalho com acesso privilegiado podem ajudar a proteger sua organização
description: Como o PAW pode aumentar a postura de segurança de sua organização
...
As PAWs (Privileged Access Workstations, Estações de Trabalho com Acesso Privilegiado) fornecem um sistema operacional dedicado para tarefas confidenciais, que é protegido contra ataques da Internet e vetores de ameaça. Separar essas tarefas e contas confidenciais das estações de trabalho e dispositivos de uso diário fornece uma proteção muito forte contra ataques de phishing, vulnerabilidades de aplicativo e sistema operacional, vários ataques de representação e ataques de roubo de credenciais, como pressionamentos de tecla registro em log, [Pass-the-hash](https://aka.ms/pth)e Pass-the-ticket.
...
Para fornecer a maior segurança, o PAWs sempre deve executar o sistema operacional mais atualizado e seguro disponível: A Microsoft recomenda enfaticamente o Windows 10 Enterprise, que inclui vários recursos de segurança adicionais não disponíveis em outras edições (em particular, [Credential Guard](https://technet.microsoft.com/library/mt483740%28v=vs.85%29.aspx) e [Device Guard](https://technet.microsoft.com/library/dn986865(v=vs.85).aspx)).
...
Os controles de segurança PAW concentram-se em reduzir o alto impacto e os riscos de alta probabilidade de comprometimento. Isso inclui a mitigação de ataques no ambiente e riscos que podem diminuir a eficácia dos controles PAW ao longo do tempo:
...
* **Risco de uso**: se for muito difícil usar uma PAW para tarefas diárias, os administradores serão motivados a criar soluções para facilitar seu trabalho. Frequentemente, essas soluções alternativas deixam as contas e a estação de trabalho administrativas vulneráveis a riscos consideráveis de segurança, portanto é fundamental envolver e capacitar os usuários da PAW para atenuar esses problemas de uso com segurança. Isso pode ser feito ouvindo seus comentários, instalando ferramentas e scripts necessários para executar seus trabalhos e garantindo que todos os funcionários administrativos saibam por que eles precisam usar um PAW, o que é um PAW e como usá-lo corretamente e com êxito.
* **Riscos de ambiente**: como muitos computadores e contas do ambiente estão expostos ao risco da Internet, direta ou indiretamente, uma PAW deve ser protegida contra ataques de ativos comprometidos no ambiente de produção. Isso requer a minimização do uso de ferramentas de gerenciamento e contas que tenham acesso ao PAWs para proteger e monitorar essas estações de trabalho especializadas.
...
O pessoal administrativo são usuários padrão também, eles precisam de um PAW, bem como de uma estação de trabalho de usuário padrão para verificar email, navegar na Web e acessar aplicativos corporativos de linha de negócios.  É essencial para o sucesso de qualquer implantação de PAW garantir que os administradores possam permanecer produtivos e seguros.  Uma solução segura que limita drasticamente a produtividade será abandonada pelos usuários em detrimento de uma que aprimora a produtividade (mesmo que isso seja feito de uma maneira insegura).
...
* Dispositivos dedicados separados por **hardware** para tarefas de usuário versus tarefas administrativas.
...
> [!NOTE]
> É importante que, em todos esses cenários, o pessoal administrativo tenha emitido uma conta de usuário padrão separada de contas administrativas designadas. As contas administrativas devem ser usadas apenas no sistema operacional administrativo da PAW.
...
#### <a name="simultaneous-use---adding-a-local-user-vm"></a>Uso simultâneo – adicionando uma VM de usuário local
...
Com o Hyper-V do Windows 10, uma máquina virtual convidada (também executando o Windows 10) pode ter uma experiência de usuário rica, incluindo aplicativos de som, vídeo e comunicações da Internet, como o Skype for Business.
...
#### <a name="simultaneous-use---adding-remoteapp-rdp-or-a-vdi"></a>Uso simultâneo-adicionando RemoteApp, RDP ou um VDI
...
A Microsoft usa a abordagem de arquitetura de PAW internamente, em nossos sistemas, e com nossos clientes. A Microsoft usa estações de trabalho administrativas internamente em várias capacidades, incluindo administração da infraestrutura de ti da Microsoft, desenvolvimento e operações de infraestrutura do Microsoft Cloud Fabric e outros ativos de alto valor.
...
Para obter mais detalhes sobre a floresta administrativa do ESAE, consulte a seção *abordagem de design da floresta administrativa do Esae* em protegendo o material de referência de [acesso privilegiado](../securing-privileged-access/securing-privileged-access-reference-material.md#esae-administrative-forest-design-approach).
...
* **Privilégios administrativos** -PAWs fornecem maior segurança para tarefas e funções administrativas de ti de alto impacto. Essa arquitetura pode ser aplicada à administração de vários tipos de sistemas, incluindo Domínios e Florestas do Active Directory, locatários do Microsoft Azure Active Directory, locatários do Office 365, PCN (Redes de controle de processo), sistemas SCADA (Aquisição de dados e controle de supervisão), caixas eletrônicos e dispositivos para pontos de venda.
* **Operadores de informações de alta sensibilidade** -a abordagem usada em um Paw também pode fornecer proteção para tarefas e funcionários altamente confidenciais do operador de informações, como aqueles que envolvem a atividade de aquisição e fusão de pré-registro, pré-lançamento financeira relatórios, presença de mídia social organizacional, comunicações executivas, segredos comerciais não patenteados, pesquisa confidencial ou outros dados proprietários ou confidenciais. Este guia não discute detalhadamente as configurações desses cenários de trabalhador de informações, nem inclui esse cenário nas instruções técnicas.
...
* [**Fase 1-implantação imediata para administradores de Active Directory**](#phase-1-immediate-deployment-for-active-directory-administrators) isso fornece um Paw rapidamente que pode proteger o domínio local e as funções de administração de floresta
* [**Fase 2-estender o Paw para todos os administradores**](#phase-2-extend-paw-to-all-administrators) isso permite a proteção para administradores de serviços de nuvem, como o Office 365 e o Azure, servidores corporativos, aplicativos empresariais e estações de trabalho
* [**Fase 3-segurança de Paw avançada**](#phase-3-extend-and-enhance-protection) aborda as proteções e considerações adicionais para a segurança do Paw

### <a name="why-dedicated-workstations"></a>Por que estações de trabalho dedicadas?
...
Esse ambiente de ameaças exige que as organizações adotem uma postura de segurança "supor a violação" ao criar proteções para ativos de alto valor, como contas administrativas e ativos comerciais confidenciais. Esses ativos de alto valor precisam ser protegidos contra ameaças diretas da Internet e contra ataques vindos de outras estações de trabalho, servidores e dispositivos no ambiente.
...
Esta seção compara a segurança das abordagens alternativas às PAWs e contém informações sobre como integrar corretamente essas abordagens dentro de uma arquitetura de PAW. todas essas abordagens trazem riscos significativos quando implementadas isoladamente, mas podem agregar valor a uma implementação de PAW em alguns cenários.

#### <a name="credential-guard-and-windows-hello-for-business"></a>Credential Guard e Windows Hello para empresas

Apresentado no Windows 10, o [Credential Guard](https://technet.microsoft.com/library/mt483740%28v=vs.85%29.aspx) usa segurança baseada em virtualização e em hardware para atenuar ataques comuns de roubo de credenciais, como Passagem de Hash, protegendo as credenciais derivadas. A chave privada para as credenciais usadas pelo [Windows Hello para empresas](https://aka.ms/passport) também pode ser protegida por hardware Trusted Platform Module (TPM).

Essas são atenuações poderosas, mas as estações de trabalho ainda podem estar vulneráveis a determinados ataques, mesmo se as credenciais estiverem protegidas pelo Credential Guard ou pelo Windows Hello para empresas. Os ataques podem incluir privilégios de abusando e o uso de credenciais diretamente de um dispositivo comprometido, reutilização de credenciais roubadas anteriormente antes de habilitar a proteção de credenciais e abuso de ferramentas de gerenciamento e configurações de aplicativo fracas na estação de trabalho.
...
Uma máquina virtual administrativa (VM de administrador) é um sistema operacional dedicado para tarefas administrativas hospedadas em uma área de trabalho de usuário padrão. Embora essa abordagem seja semelhante à PAW no fornecimento de um sistema operacional dedicado para tarefas administrativas, tem uma falha fatal: a VM administrativa depende do desktop de usuário padrão para sua segurança.
...
A arquitetura PAW não permite hospedar uma VM de administrador em uma estação de trabalho de usuário, mas uma VM de usuário com uma imagem corporativa padrão pode ser hospedada em um PAW de administrador para fornecer à equipe um único PC para todas as responsabilidades.

![Diagrama da arquitetura de PAW](../media/privileged-access-workstations/PAWFig9.JPG)

#### <a name="shielded-vm-based-paws"></a>PAWs com base em VM blindada

Uma variante segura do modelo de VM administrativa é usar [máquinas virtuais blindadas](../../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms.md) para hospedar uma ou mais VMs de administrador juntamente com uma VM de usuário.
As VMs blindadas são projetadas para executar cargas de trabalho seguras em um ambiente onde usuários potencialmente não confiáveis ou código podem estar em execução na área de trabalho de usuário padrão do computador físico.
Uma VM blindada tem um TPM virtual que permite criptografar seus próprios dados em repouso, e vários controles administrativos, como o acesso de console básico, o PowerShell Direct, e a capacidade de depurar a VM são desabilitados para isolar ainda mais a VM da área de trabalho de usuário padrão e outras VMs.
As chaves de uma VM blindada são armazenadas em um servidor de gerenciamento de chaves confiável, o que exige que o dispositivo físico atestar sua identidade e integridade antes de liberar uma chave para iniciar a VM.
...
Como as VMs blindadas são isoladas umas das outras e da área de trabalho de usuário padrão, é aceitável executar várias VMs PAW blindadas em um único host, mesmo quando essas VMs gerenciam diferentes camadas.
...
> Para obter mais informações sobre o modelo de camada, consulte [https://aka.ms/tiermodel](https://aka.ms/tiermodel) para obter mais informações sobre grupos de camada 0, consulte equivalência da camada 0 na [proteção do material de referência de acesso privilegiado](../securing-privileged-access/securing-privileged-access-reference-material.md).

Para obter mais informações sobre como implantar o gerenciamento de acesso privilegiado (PAM) Microsoft Identity Manager (MIM), consulte [https://aka.ms/mimpamdeploy](https://aka.ms/mimpamdeploy)
...
## <a name="paw-phased-implementation"></a>Implementação em fases do PAW
...
* [Fase 1-implantação imediata para administradores de Active Directory](#phase-1-immediate-deployment-for-active-directory-administrators)
* [Fase 2-estender o PAW para todos os administradores](#phase-2-extend-paw-to-all-administrators)
* [Fase 3-segurança de PAW avançada](#phase-3-extend-and-enhance-protection)
...
### <a name="phase-1-immediate-deployment-for-active-directory-administrators"></a>Fase 1: Implantação imediata para administradores de Active Directory

Finalidade: Fornece um PAW rapidamente que pode proteger funções de administração de domínio e floresta locais.

Escopo: Administradores de camada 0, incluindo administradores corporativos, administradores de domínio (para todos os domínios) e administradores de outros sistemas de identidade autoritativa.
...
* Unidades organizacionais (UO)
   * Seis novas UOs de nível superior:  ADM Agrupa Servidores da camada 1; Estações Contas de usuário; e a quarentena do computador.  Cada UO de nível superior conterá várias UOs filhas.
* Grupos
   * Seis novos grupos globais habilitados para segurança:  Manutenção da replicação da camada 0; Manutenção do servidor da camada 1; Operadores da central de serviços; Manutenção da estação de trabalho; Usuários do PAW; Manutenção PAW.

Você também criará vários objetos de política de Grupo: Configuração do PAW – computador; Configuração do PAW – usuário; RestrictedAdmin obrigatório-computador; PAW restrições de saída; Restringir o logon da estação de trabalho; Restringir o logon do servidor.
...
4. **Adquira e valide o Windows 10 Enterprise Edition e o software de aplicativo necessários**. Obtenha o software necessário para a PAW e valide-o usando a orientação em [Origem limpa para mídia de instalação](https://aka.ms/cleansource).
...
1. Baixe a biblioteca de scripts de PAW na [Galeria do TechNet](https://aka.ms/PAWmedia)
   ...
   > [!NOTE]
   > Baixe todos os arquivos e salve-os no mesmo diretório e execute-os na ordem especificada abaixo.  Create-PAWGroups depende da estrutura de UO criada por Create-PAWOUs, e Set-PAWOUDelegation depende dos grupos criados por Create-PAWGroups.
...
#### <a name="move-tier-0-accounts-to-the-admintier-0accounts-ou"></a>Mover contas da camada 0 para a UO Admin\Tier 0 \ accounts
...
#### <a name="create-paw-configuration---computer-group-policy-object-gpo"></a>Criar objeto de política de grupo "configuração de PAW-computador" (GPO)

Nesta seção, você criará um novo GPO "configuração do PAW-computador" que fornecerá proteções específicas para esses PAWs e vinculá-lo à UO dos dispositivos da camada 0 ("dispositivos" na camada 0 \ administrador).
...
2. **Restringir a Associação no Grupo Local** - essa configuração garantirá que a associação dos grupos de administradores locais na estação de trabalho esteja sempre vazia
   ...
   2. Conclua as etapas acima para os seguintes grupos adicionais:
      ...
      * Replicadores
   3. **Paw restrições de logon** – essa configuração limitará as contas que podem fazer logon no Paw. Execute as etapas abaixo para definir essa configuração:
      ...
      2. Selecione definir estas configurações de política e adicionar "usuários PAW" e administradores (novamente, não use o botão procurar para selecionar administradores).
   4. **Bloquear Tráfego de Rede de Entrada** - essa configuração garante que nenhum tráfego de rede de entrada não solicitado tenha permissão na PAW. Execute as etapas abaixo para definir essa configuração:
      1. Acesse Configuração do Computador\Políticas\Configurações do Windows\Configurações de Segurança\Firewall do Windows com Segurança Avançada\Firewall do Windows com Segurança Avançada e execute estas etapas:
         ...
         4. Clique em **OK**.
            ...
            > As configurações no arquivo WFW permitirão o firewall no modo "Bloquear – Padrão" para todos os perfis de firewall, desativarão a mesclagem de regra e habilitarão o registro em log de pacotes descartados e bem-sucedidos. Essas configurações bloquearão o tráfego não solicitado e ainda permitirão a comunicação bidirecional em conexões iniciadas do PAW, impedirá que os usuários com acesso administrativo local criem regras de firewall locais que substituam as configurações de GPO e Verifique se o tráfego dentro e fora do PAW está registrado.
   ...
   5. **Configurar o Windows Update para o WSUS** - execute as etapas abaixo para alterar as configurações a fim de configurar o Windows Update para as PAWs:
      1. Acesse configuração do computador \ Modelos Administrativos\Componentes do Windows\windows updates e siga as etapas abaixo:
         ...
         4. Habilitar opção especifique a política de **local do serviço de atualização da intranet da Microsoft** e especifique em ambas as opções a URL do servidor do Esae WSUS.
   6. Vincule o GPO "configuração do PAW-computador" da seguinte maneira:

         |Política|Local do link|
         |-----|---------|
         |Configuração da PAW - Computador |Admin\Camada 0\Dispositivos|

#### <a name="create-paw-configuration---user-group-policy-object-gpo"></a>Criar objeto de política de grupo (GPO) de "configuração de PAW-usuário"

Nesta seção, você criará um novo GPO "PAW Configuration-User", que fornecerá proteções específicas para esses PAWs e link para a UO de contas da camada 0 ("contas" na camada 0 \ administrador).
...
1. **Bloquear navegação na Internet** - Para deter a navegação acidental na Internet, isso definirá um endereço de proxy de um endereço de loopback (127.0.0.1).
   1. Vá para o usuário Configuration\Preferences\Windows Settings\Registry. Clique com o botão direito do mouse em registro, selecione **novo** **Item de registro**  >  e defina as seguintes configurações:
      1. Ação:  Substituir
      2. Sessão HKEY_CURRENT_USER
      3. Caminho da chave:  Configurações de Software\Microsoft\Windows\CurrentVersion\Internet
   ...
   2. Vá para o usuário Configuration\Preferences\Windows Settings\Registry. Clique com o botão direito do mouse em registro, selecione **novo** **Item de registro**  >  e defina as seguintes configurações:

      * Ação: Substituir
      * Sessão HKEY_CURRENT_USER
      * Caminho da chave: Configurações de Software\Microsoft\Windows\CurrentVersion\Internet
...
2. Vá para User configuração Administrativos\Componentes do Windows\internet Explorer e habilite as opções abaixo. Essas configurações impedirão que os administradores substituam as configurações de proxy manualmente.
...
#### <a name="restrict-administrators-from-logging-onto-lower-tier-hosts"></a>Impedir que os administradores façam logon em hosts de camada inferior
...
10. Adicional Baixe e instale o software de acesso remoto necessário. Se os administradores usarem a PAW remotamente para administração, instale o software de acesso remoto usando as diretrizes de segurança de seu fornecedor de solução de acesso remoto. Obtenha todas as mídias de instalação usando as diretrizes de Origem limpa para a mídia de instalação.

    > [!NOTE]
    > Considere atentamente todos os riscos envolvidos na permissão de acesso remoto por meio de um PAW.  Embora uma PAW móvel permita vários cenários importantes, incluindo home office, o software de acesso remoto pode ficar vulnerável a ataques e ser usado para comprometer uma PAW.
...
11. Valide a integridade do sistema da PAW revisando e confirmando que todas as configurações apropriadas foram aplicadas usando as etapas abaixo:
    1. Confirme se apenas as políticas de grupo específicas às PAWs foram aplicadas à PAW
       1. Abra um prompt de comando com privilégios elevados e insira o seguinte comando: `Gpresult /scope computer /r`
    ...
    2. Confirme que nenhuma conta de usuário adicional é membro dos grupos privilegiados na PAW usando as etapas abaixo:
       ...
       2. Além disso, usando **Editar Usuários e Grupos Locais**, verifique se os seguintes grupos têm algum membro: Operadores de backup operadores de criptografia Hyper-V administradores configuração de rede operadores usuários avançados Área de Trabalho Remota usuários replicadores

12. Adicional Se sua organização usa uma solução SIEM (gerenciamento de informações e eventos de segurança), verifique se o PAW está [configurado para encaminhar eventos para o sistema usando o WEF (encaminhamento de eventos do Windows)](http://blogs.technet.com/b/jepayne/archive/2015/11/24/monitoring-what-matters-windows-event-forwarding-for-everyone-even-if-you-already-have-a-siem.aspx) ou, de outra forma, registrado com a solução para que o Siem seja recebendo ativamente eventos e informações do PAW.  Os detalhes dessa operação poderão variar com base em sua solução SIEM.
...
13. Adicional Se você optou por implantar as interrupções para gerenciar a senha da conta de administrador local em seu PAW, verifique se a senha foi registrada com êxito.
...
### <a name="phase-2-extend-paw-to-all-administrators"></a>Fase 2: Estender PAW para todos os administradores
...
> [!NOTE]
> As instruções nesta fase presumem que a Fase 1 foi concluída totalmente.  Não inicie a fase 2 até concluir todas as etapas na fase 1.

Depois de confirmar que todas as etapas foram executadas, execute as etapas abaixo para concluir a Fase 2:

#### <a name="recommended-enable-restrictedadmin-mode"></a>Aconselhável Habilitar modo **RestrictedAdmin**

Habilite esse recurso em seus servidores e estações de trabalho existentes e, em seguida, aplique o uso desse recurso. Esse recurso exigirá que os servidores de destino estejam executando o Windows Server 2008 R2 ou posterior e estações de trabalho de destino estejam executando o Windows 7 ou posterior.
...
3. Vincule o **RestrictedAdmin** Requires - Computador aos dispositivos apropriados da Camada 1 e/ou Camada 2 usando as opções de Política a seguir:
   ...
   * Configuração da PAW - Usuário
      * -> local do link: Admin\Tier 0 \ contas
...
5. Adicional Habilite a conectividade com os serviços de nuvem. Esta etapa permite o gerenciamento de serviços de nuvem como o Azure e o Office 365 com as garantias de segurança apropriadas. Esta etapa também é necessária para o Microsoft Intune, a fim de gerenciar as PAWs.
   ...
   * Para habilitar a conectividade para os serviços da PAW, execute as etapas abaixo:
   ...
   1. Configure a PAW para permitir somente destinos da Internet autorizados.  Ao estender sua implantação do PAW para habilitar a administração de nuvem, você precisa permitir o acesso a serviços autorizados enquanto filtra o acesso da Internet aberta, onde os ataques podem ser montados com mais facilidade em seus administradores.

      1. Criar grupo de **Administradores de serviços de nuvem** e adicionar todas as contas a ele que exigem acesso aos serviços de nuvem na Internet.
      ...
      3. Após a configuração do arquivo *proxy.pac*, atualize o GPO Configuração da PAW - Usuário.
         1. Vá para o usuário Configuration\Preferences\Windows Settings\Registry. Clique com o botão direito do mouse em registro, selecione **novo** **Item de registro**  >  e defina as seguintes configurações:
            1. Ação: Substituir
            2. Sessão HKEY_ CURRENT_USER
            3. Caminho da chave: Configurações de Software\Microsoft\Windows\CurrentVersion\Internet
            4. Nome do valor: Configural
            ...
            6. Dados do valor: Insira a URL completa para o arquivo *proxy. PAC* , incluindo http://e o nome do arquivo, por exemplo http://proxy.fabrikam.com/proxy.pac.  A URL também pode ser uma URL de rótulo único – por exemplo, http://proxy/proxy.pac
               ...
               > [!NOTE]
               > O arquivo PAC também pode estar hospedado em um compartilhamento de arquivos, com a sintaxe file://server.fabrikan.com/share/proxy.pac, mas isso exige a permissão do protocolo file://. Consulte a "Observação: Scripts de proxy File://-based preteridos "deste blog [sobre configuração de proxy da Web](http://blogs.msdn.com/b/ieinternals/archive/2013/10/11/web-proxy-configuration-and-ie11-changes.aspx) para obter detalhes adicionais sobre como configurar o valor de registro necessário.
...
7. Identifique e obtenha com segurança o software e os aplicativos necessários para administração.  Isso é semelhante ao trabalho realizado na Fase 1, mas com um escopo mais amplo devido à quantidade maior de aplicativos, serviços e sistemas que estão sendo protegidos.
   ...
   > [!NOTE]
   > Certifique-se de proteger esses novos aplicativos (incluindo navegadores da Web), optando-os nas proteções fornecidas pelo Windows Defender Exploit Guard.
...
8. Adicional Baixe e instale os agentes de gerenciamento necessários.
...
12. Considere as necessidades culturais, operacionais, comunicações e de treinamento que acompanham uma implantação de PAW estendida.   Uma alteração considerável em um modelo administrativo naturalmente exigirá um certo nível de gerenciamento de mudanças, e é essencial agregar isso ao próprio projeto de implantação.  Considere no mínimo o seguinte:
    ...
    * Como você garantirá a conformidade com o novo processo?  Embora o modelo PAW inclua vários controles técnicos para evitar a exposição de credenciais com privilégios, é impossível impedir totalmente toda a exposição possível, puramente usando controles técnicos.  Por exemplo, embora seja possível evitar que um administrador faça logon em um desktop de usuário com credenciais privilegiadas, o simples ato de tentar o logon pode expor as credenciais a algum malware instalado desse desktop de usuário.  Portanto, é essencial que você descreva, além dos benefícios do modelo da PAW, os riscos da não conformidade.  Isso deve ser complementado por auditoria e alertas, para que a exposição de credencial possa ser detectada e resolvida rapidamente.

### <a name="phase-3-extend-and-enhance-protection"></a>Fase 3: Estender e aprimorar a proteção

Escopo: Essas proteções aprimoram os sistemas criados na fase 1, fortalecendo a proteção básica com recursos avançados, incluindo autenticação multifator e regras de acesso à rede.
...
1. **Habilite a autenticação multifator para contas privilegiadas**.  A autenticação multifator reforça a segurança da conta exigindo que o usuário forneça um token físico além das credenciais.  A autenticação multifator complementa muito bem as políticas de autenticação, mas não depende de políticas de autenticação para a implantação (e, da mesma forma, as políticas de autenticação não exigem autenticação multifator).  A Microsoft recomenda o uso de uma destas formas de autenticação multifator:
   ...
   * **Cartão inteligente**: Um cartão inteligente é um dispositivo físico portátil e resistente a adulterações que fornece uma segunda verificação durante o processo de logon do Windows.  Ao exigir que um indivíduo possua um cartão para o logon, você pode reduzir o risco de reutilização remota das credenciais roubadas.  Para saber mais sobre o logon com cartão inteligente no Windows, consulte o artigo [Visão geral do cartão inteligente](https://technet.microsoft.com/library/hh831433.aspx).
   * **Cartão inteligente virtual**:  Um cartão inteligente virtual fornece os mesmos benefícios de segurança que os cartões inteligentes físicos, com o benefício adicional de ser vinculado a um hardware específico.  Para obter detalhes sobre os requisitos de implantação e hardware, consulte os artigos, [visão geral do cartão inteligente virtual](https://technet.microsoft.com/library/dn593708.aspx) e [Get iniciados com cartões inteligentes virtuais: Guia explicativo @ no__t-0.
   * **Windows Hello para empresas**: O Windows Hello para empresas permite que os usuários se autentiquem em um conta Microsoft, uma conta de Active Directory, uma conta do Microsoft Azure Active Directory (Azure AD) ou um serviço que não seja da Microsoft que dê suporte à autenticação FIDO (Fast ID online). Após uma verificação inicial em duas etapas durante o registro do Windows Hello para empresas, um Windows Hello para empresas é configurado no dispositivo do usuário e o usuário define um gesto, que pode ser o Windows Hello ou um PIN. As credenciais do Windows Hello para empresas são um par de chaves assimétricas, que pode ser gerado em ambientes isolados de TPMs (módulos de plataforma confiável).
      Para obter mais informações sobre o Windows Hello para empresas, leia o artigo sobre o [Windows Hello para empresas](https://docs.microsoft.com/windows/security/identity-protection/hello-for-business/hello-identity-verification) .
   * **Autenticação multifator do Azure**:  A autenticação multifator do Azure (MFA) fornece a segurança de um segundo fator de verificação, bem como proteção aprimorada por meio de monitoramento e análise baseada em aprendizado de máquina.  A MFA do Azure pode proteger não só os administradores do Azure, mas muitas outras soluções, incluindo aplicativos Web, Azure Active Directory e soluções locais, como o acesso remoto e Área de Trabalho Remota.  Para saber mais sobre a autenticação multifator do Azure, consulte o artigo [Autenticação multifator](https://azure.microsoft.com/services/multi-factor-authentication).

2. **Aplicativos confiáveis da lista branca usando o controle de aplicativos do Windows Defender e/ou o AppLocker**.  Ao limitar a capacidade do código não confiável ou não assinado executar em uma PAW, você reduz ainda mais a probabilidade de atividade mal-intencionada e comprometimento.  O Windows inclui duas opções principais para controle de aplicativo:
   ...
   * **Controle de aplicativos do Windows Defender**: o novo recurso de controle de aplicativo do Windows Defender fornece controle de aplicativo baseado em hardware aprimorado que, ao contrário do AppLocker, não pode ser substituído no dispositivo afetado.  Como o AppLocker, o controle de aplicativos do Windows Defender pode ser controlado por meio da diretiva de grupo e direcionado a usuários específicos.  Para obter mais informações sobre como restringir o uso do aplicativo com o controle de aplicativos do Windows Defender, consulte o [Guia de implantação do controle de aplicativos do Windows Defender](https://docs.microsoft.com/windows/security/threat-protection/windows-defender-application-control/windows-defender-application-control-deployment-guide).
...
## <a name="managing-and-updating"></a>Gerenciando e atualizando
...
|Gerenciar PAWs com ferramentas de gerenciamento existentes|-Cria um risco significativo para comprometer o PAWs, a menos que a infraestrutura de gerenciamento existente seja apresentada no nível de segurança da observação de PAWs **:**     A Microsoft geralmente não incentiva essa abordagem, a menos que sua organização tenha um motivo específico para usá-la. Em nossa experiência, normalmente há um custo muito alto para trazer todas essas ferramentas (e suas dependências de segurança) até o nível de segurança do PAWs.<br />-   A maioria dessas ferramentas oferece visibilidade e controle de configuração, implantação de software e atualizações de segurança|
...
Uma [malha protegida](https://aka.ms/shieldedvms) pode ser usada para executar cargas de trabalho do Paw em uma máquina virtual blindada em um laptop ou servidor de salto.
A adoção dessa abordagem requer uma infraestrutura extra e etapas operacionais, mas pode facilitar a reimplantação de imagens de PAW em intervalos regulares e permite consolidar várias PAWs diferentes em camadas (ou classificações) em máquinas virtuais em execução lado a lado em um único dispositivo.
Para obter uma explicação completa sobre a topologia de malha protegida e as promessas de segurança, consulte a [documentação da malha protegida](https://aka.ms/shieldedvms).

### <a name="changes-to-the-paw-gpos"></a>Alterações nos GPOs do PAW

Ao usar PAWs com base em VM blindadas, as [configurações de GPO recomendadas](#create-paw-configuration---computer-group-policy-object-gpo) definidas acima precisarão ser modificadas para dar suporte ao uso de máquinas virtuais.

1. Crie uma nova UO para os hosts de PAW físicos. Os PAWs físicos e virtuais têm requisitos de segurança diferentes e devem ser separados em Active Directory de acordo.
2. O GPO do computador PAW deve ser vinculado às UOs PAW física e virtual.
3. Crie um novo GPO para o PAW físico para adicionar os usuários do PAW ao grupo Administradores do Hyper-V. Isso é necessário para permitir que os administradores se conectem às VMs de administração e ligue/desative-os conforme necessário. É importante que o usuário que faz logon no PAW físico não tenha direitos de administrador, acesso à Internet ou a capacidade de copiar dados de máquinas virtuais mal-intencionadas de compartilhamentos de rede ou dispositivos de armazenamento externo no PAW físico.
4. Crie um novo GPO para as VMs de administração para adicionar usuários do PAW ao grupo Área de Trabalho Remota usuários. Isso permitirá que os usuários usem sessões de console avançadas do Hyper-V, que oferecem uma melhor experiência do usuário e habilita a passagem de cartão inteligente para a VM.

### <a name="set-up-the-host-guardian-service"></a>Configurar o serviço guardião de host

O serviço guardião de host é responsável por atestar a identidade e a integridade de um dispositivo PAW físico.
...
Isso ajuda a proteger as VMs blindadas, que executam cargas de trabalho confiáveis para gerenciar recursos em camadas, desde ameaças de ambiente de área de trabalho do usuário.

Como o HGS é responsável por determinar quais dispositivos podem executar VMs PAW, ele é considerado um recurso de camada 0.
Ele deve ser implantado junto com outros recursos da camada 0 e protegido contra acesso físico e lógico não autorizado.
O HGS é uma função clusterizada, facilitando a expansão para qualquer implantação de tamanho.
A regra geral é planejar um servidor HGS para cada 1.000 dispositivos que você tem, com um mínimo de 3 nós.

1. Para instalar seu primeiro servidor HGS, comece com o artigo [instalar a floresta HgS-bastiões](../../security/guarded-fabric-shielded-vm/guarded-fabric-install-hgs-in-a-bastion-forest.md) e INGRESSE em HgS no domínio da camada 0.
...
Qualquer pessoa em posse dos certificados de criptografia e autenticação HGS pode descriptografar uma VM blindada, portanto, se você tiver acesso a um módulo de segurança de hardware para proteger chaves privadas, é recomendável que você gere esses certificados usando um HSM.
Para uma segurança mais forte, selecione um tamanho de chave maior ou igual a 4096 bits.
...
A inicialização configura os serviços Web de atestado e proteção de chave usados pelo seu PAWs.
...
4. Siga as etapas para [adicionar mais nós](../../security/guarded-fabric-shielded-vm/guarded-fabric-configure-additional-hgs-nodes.md) ao segundo, terceiro e nós HgS adicionais.

5. Se o seu servidor HGS estiver executando o Windows Server 2019 ou posterior, você poderá habilitar um recurso opcional para armazenar em cache as chaves para VMs blindadas no PAWs para que elas possam ser usadas offline. As chaves são lacradas para a configuração de segurança atual do sistema para impedir que alguém use chaves armazenadas em cache em outro computador ou no mesmo computador em um estado inseguro. Isso pode ser uma solução útil se os usuários do PAW viajam sem acesso à Internet, mas ainda precisam ser capazes de fazer logon em suas VMs do PAW. Para usar esse recurso, execute o seguinte comando em qualquer servidor HGS:

      ```powershell
      Set-HgsKeyProtectionConfiguration -AllowKeyMaterialCaching:$true
      ```

### <a name="set-up-the-physical-paw-device"></a>Configurar o dispositivo físico PAW

O dispositivo físico PAW é considerado não confiável por padrão na solução de malha protegida.
Ele pode provar que é confiável durante o processo de atestado, após o qual ele pode obter as chaves necessárias para iniciar uma VM de administrador blindada.
O dispositivo deve ser capaz de executar o Hyper-V e ter a inicialização segura e um TPM 2,0 habilitado para atender aos [pré-requisitos de host protegidos](../../security/guarded-fabric-shielded-vm/guarded-fabric-guarded-host-prerequisites.md).
A versão mínima do sistema operacional para dar suporte a todas as funcionalidades do PAW é **Windows 10 versão 1803**.

O PAW físico deve ser configurado como qualquer outro, com exceção de que qualquer usuário do PAW precisará ser administradores do Hyper-V para poder ativar a VM do administrador e conectar-se a ela.
Em seu ambiente de sala de limpeza, você precisará criar uma configuração de ouro para cada combinação exclusiva de hardware/software que estiver implantando como hosts protegidos para VMs de administração.
Em cada configuração de ouro, conclua as seguintes tarefas:

1. Instale as atualizações mais recentes para Windows, drivers e firmware no computador, bem como quaisquer agentes de gerenciamento ou monitoramento de terceiros.
2. [Capture as informações de linha de base necessárias](../../security/guarded-fabric-shielded-vm/guarded-fabric-tpm-trusted-attestation-capturing-hardware.md), incluindo o identificador exclusivo do TPM (chave de endosso), as medidas de inicialização (log de TCG) e a política de integridade de código para o computador.
3. Copie esses artefatos para um servidor HGS e execute os comandos de atestado HGS no artigo anterior para registrar o host. Se todos os seus hosts usarem a mesma política de integridade de código e/ou usarem a mesma configuração de hardware, você só precisará registrar a política de integridade de código/log de TCG uma vez.
...
Esta máquina se tornará a imagem Golden para uma VM de administrador.
Você pode criar mais de um disco de modelo para ter ferramentas especializadas disponíveis em diferentes contextos.

Quando a VM estiver configurada como desejado, execute `C:\Windows\System32\sysprep\sysprep.exe` e escolha **generalizar** o disco. **Desligue** o sistema operacional quando a generalização for concluída.
...
O arquivo de dados de blindagem inclui:
   ...
   - Uma lista de assinaturas de disco confiável para implantação. Os arquivos de dados de blindagem só liberarão seus segredos para as VMs criadas usando mídia de origem autorizada.
   - Uma política de segurança que determina se as proteções extras devem ser colocadas em vigor para proteger a VM do host e se a VM tem permissão para ser movida para outra máquina. As VMs de administrador do PAW sempre devem ser totalmente blindadas.
...
Consulte o [artigo arquivo de dados de blindagem](../../security/guarded-fabric-shielded-vm/guarded-fabric-tenant-creates-shielding-data.md) para obter as etapas sobre como criar um arquivo de dados de blindagem.
...
Eles podem ser usados em um cenário de interrupção de emergência para inicializar uma VM blindada sem a presença do HGS.

É altamente recomendável que a blindagem de dados para VMs de administração do PAW inclua a configuração para bloquear uma VM para o primeiro host físico em que ela é inicializada.
Isso impedirá que alguém mova uma VM de administrador de um PAW para outro PAW no mesmo ambiente.
...
Depois que o disco de modelo e o arquivo de dados de blindagem estiverem prontos, você poderá implantar uma VM de administrador em qualquer PAW que tenha sido registrado com HGS.
...
[Participando dos serviços segurança cibernéticas da Microsoft](https://www.microsoft.com/en-us/microsoftservices/campaigns/cybersecurity-protection.aspx)

[Taste do Premier: Como mitigar o Pass-the-hash e outras formas de roubo de credenciais @ no__t-0
...
[Protegendo ativos de alto valor com estações de trabalho de administração segura](https://msdn.microsoft.com/library/mt186538.aspx)
...
[Processos e recursos do modo de usuário isolado no Windows 10 com Logan Gabriel (Channel 9)](http://channel9.msdn.com/Blogs/Seth-Juarez/Isolated-User-Mode-Processes-and-Features-in-Windows-10-with-Logan-Gabriel)
...
[Reduzindo o roubo de credenciais usando o modo de usuário isolado do Windows 10 (canal 9)](https://channel9.msdn.com/Blogs/Seth-Juarez/Mitigating-Credential-Theft-using-the-Windows-10-Isolated-User-Mode)

[Habilitando validação de KDC estrita no Kerberos do Windows](https://www.microsoft.com/en-us/download/details.aspx?id=6382)

[O que há de novo na autenticação Kerberos para o Windows Server 2012](https://technet.microsoft.com/library/hh831747.aspx)
