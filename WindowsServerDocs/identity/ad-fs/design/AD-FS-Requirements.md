...
-   Para acesso à extranet, você deve implantar a parte do serviço de função do proxy de aplicativo Web \- da função de servidor de acesso remoto do Windows Server &reg; 2012 R2. Não há suporte para versões anteriores de um proxy de servidor de Federação com AD FS no Windows Server &reg; 2012 R2.  
...
-   Para que a autenticação Kerberos funcione entre \- clientes ingressados no domínio e AD FS, o ' HOST \/<\_ nome do serviço ADFS \_> ' deve ser registrado como um SPN na conta de serviço. Por padrão, AD FS configurará isso ao criar um novo farm de AD FS se ele tiver permissões suficientes para executar essa operação.  
...
Além disso, se a autenticação de certificado de usuário do cliente \( clientTLS de autenticações usando certificados de usuário X509 \) for necessária, AD FS no Windows Server 2012 R2 exigirá que a porta TCP 49443 seja habilitada para entrada no firewall entre os clientes e o proxy de aplicativo Web. Isso não é necessário no firewall entre o proxy de aplicativo Web e os servidores de Federação \) .  
...
-   Para acesso à intranet, todos os clientes que acessam AD FS serviço na intranet da rede corporativa interna \( \) devem ser capazes de resolver o nome do serviço de AD FS \( fornecido pelo certificado SSL \) para o balanceador de carga para os servidores AD FS ou o servidor AD FS.  
  
-   Para acesso à extranet, todos os clientes que acessam AD FS serviço de fora da extranet da rede corporativa \( \/ \) devem ser capazes de resolver o nome do serviço de AD FS \( fornecido pelo certificado SSL \) para o balanceador de carga para os servidores de proxy de aplicativo Web ou o servidor proxy de aplicativo Web.  
...
-   Além disso, o HOST \/<\_ nome do serviço ADFS \_> SPN deve ser definido na conta de serviço na qual o farm de AD FS é executado.  
...
|\- [Protocolo aes256 http: \/ \/ www.w3.org \/ 2001 \/ 04 \/ xmlenc \# aes256 \- CBC](http://www.w3.org/2001/04/xmlenc#aes256-cbc)|256|**Padrão**. Algoritmo com suporte para criptografar o token de segurança.|  
...
|RsaV15KeyWrap \- [http: \/ \/ www.w3.org \/ 2001 \/ 04 \/ xmlenc \# RSA \- 1 \_ 5](http://www.w3.org/2001/04/xmlenc#rsa-1_5)|1024|Algoritmo com suporte para criptografar a chave simétrica que criptografa o token de segurança.|  
