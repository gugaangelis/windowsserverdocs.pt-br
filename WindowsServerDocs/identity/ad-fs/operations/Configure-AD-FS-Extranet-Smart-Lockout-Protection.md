...
ms.locfileid: "69560490"
---
# <a name="ad-fs-extranet-lockout-and-extranet-smart-lockout"></a>Bloqueio de extranet e bloqueio inteligente de extranet AD FS
...
- **UnknownLocation**: Se uma solicitação que chega tiver pelo menos um IP não presente na lista de "FamiliarLocation" existente, a solicitação será tratada como um local "desconhecido". Isso é para lidar com cenários de proxy, como a autenticação herdada do Exchange Online, na qual os endereços do Exchange Online lidam com solicitações bem-sucedidas e com falha.  
...
- **ExtranetLockoutMode**: Controla somente o modo de bloqueio do controle inteligente de extranet em log
    ...
    - **ADFSSmartLockoutEnforce**: Suporte completo para bloqueio de solicitações de autenticação desconhecidas quando os limites são atingidos.
...
- **Falha no logon**: Se o logon falhar, o badPwdCount será aumentado. O usuário entrará em um estado de bloqueio se o invasor enviar mais senhas inadequadas ao sistema do que o limite permite. (badPwdCount > ExtranetLockoutThreshold)  
...
`Get-ADFSAccountActivity user@contoso.com`
  ...
  Properties
    ...
    - LastFailedAuthFamiliar: Se a autenticação não tiver sido bem-sucedida em um local familiar, o LastFailedAuthUnknown será definido para a hora da autenticação malsucedida
    - LastFailedAuthUnknown: Se a autenticação não tiver sido bem-sucedida de um local desconhecido, o LastFailedAuthUnknown será definido para a hora da autenticação malsucedida
...
### <a name="connect-health"></a>Conectar integridade
A maneira recomendada para monitorar a atividade de conta de usuário é por meio do Connect Health. O Connect Health gera relatórios para download em IPs arriscados e tentativas de senha inadequadas. Cada item no relatório de IP arriscado mostra informações agregadas sobre atividades de entrada AD FS com falha que excedem o limite designado. As notificações por email podem ser definidas para alertar os administradores assim que isso ocorrer com as configurações de email personalizáveis. Para obter informações adicionais e instruções de instalação, visite a [documentação do Connect Health](https://docs.microsoft.com/azure/active-directory/hybrid/how-to-connect-health-adfs).
