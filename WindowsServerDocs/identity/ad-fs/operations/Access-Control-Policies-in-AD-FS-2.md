...
|                                                         Usado para estabelecer que a solicitação é de um cliente com um IP no intervalo aceitável definido.                                                         | Não existe ([type = = "<https://schemas.microsoft.com/2012/01/requestcontext/claims/x-ms-forwarded-client-ip>", valor = ~ "Regex de endereço IP público fornecido pelo cliente"]) |
...
AD FS eventos de rastreamento são registrados no log de depuração do AD FS 2,0. Para habilitar o rastreamento, consulte [Configurar o rastreamento de depuração para AD FS 2,0](https://technet.microsoft.com/library/adfs2-troubleshooting-configuring-computers.aspx).
