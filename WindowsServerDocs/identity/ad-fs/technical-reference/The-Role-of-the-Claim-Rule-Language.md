...
Por exemplo, na ilustração a seguir, a declaração de entrada é adicionada ao conjunto de declarações de entrada definido pelo mecanismo de emissão de declarações. Quando a primeira regra de declaração personalizada é executada e os critérios de usuário de domínio for atendida, o mecanismo de emissão de declarações processa a lógica da regra usando a instrução de adição e o valor de **Editor** é adicionado ao conjunto de declarações de entrada. Como o valor de Editor está presente no conjunto de declarações de entrada, regra 2 pode processar a declaração do problema em sua lógica com êxito e gerar um novo valor de **Hello**, que é adicionado de saída de conjunto de declarações e à entrada de conjunto para uso por declaração a próxima regra no conjunto de regras. A regra 3 agora pode usar todos os valores que estão presentes na declaração de entrada definido como entrada para sua lógica de processamento.  
