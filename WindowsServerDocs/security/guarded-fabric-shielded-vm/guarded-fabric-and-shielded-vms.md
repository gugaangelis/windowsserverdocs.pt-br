...
Segurança de virtualização é uma área de grande investimento no Hyper-V. Além de proteger hosts ou outras máquinas virtuais de uma máquina virtual que está executando software mal-intencionado, também é necessário proteger as máquinas virtuais de um host comprometido. Isso é um perigo fundamental para cada plataforma de virtualização hoje, seja Hyper-V, VMware ou qualquer outro. De maneira simples, se uma máquina virtual fica fora de uma organização (seja por más intenções ou acidentalmente), essa máquina virtual pode ser executada em qualquer outro sistema. Proteger os ativos de alto valor na sua organização, como controladores de domínio, servidores de arquivos confidenciais e sistemas de RH, é uma prioridade.

Para ajudar a proteger contra a malha de virtualização comprometido, introduzido o Windows Server 2016 Hyper-V VMs blindadas. Uma máquina virtual blindada é uma geração 2 VM (com suporte no Windows Server 2012 e posterior) que tem um TPM virtual, é criptografado com BitLocker e pode ser executado somente em hosts íntegros e aprovados na malha. As VMs blindadas e as malhas protegidas permitem que os provedores de serviço de nuvem ou administradores de nuvens privadas empresariais forneçam um ambiente mais seguro para VMs locatárias. 
...
O HGS dá suporte a modos de Atestado diferentes para uma malha protegida:

- Atestado de TPM confiável (baseado em hardware)
- Atestado de chaves do host (com base nos pares de chave assimétricos)

O Atestado de TPM confiável é recomendado porque oferece garantias mais fortes, conforme explicado na tabela a seguir, mas exige que os hosts do Hyper-V tenham TPM 2.0. Se você atualmente não tiver TPM 2.0 ou qualquer TPM, você pode usar o atestado de chaves do host. Se decidir mover de Atestado de TPM confiável ao adquirir novo hardware, você poderá alternar o modo de atestado do Serviço Guardião de Host com pouca ou nenhuma interrupção para sua malha.

| **Modo de Atestado escolhido para hosts**                                            | **Garantias de host** |
|-------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|**Atestado de TPM confiável:** Oferece as proteções mais fortes de possíveis, mas também requer mais etapas de configuração. Firmware e hardware do host devem incluir TPM 2.0 e UEFI 2.3.1 com inicialização segura habilitada. | Hosts protegidos são aprovados com base em sua identidade TPM, sequência de inicialização medida e políticas de integridade de código para garantir que eles são executados apenas código aprovado.| 
| **Atestado de chaves do host:** Pretende dar suporte a hardware de host existente onde TPM 2.0 não está disponível. Requer menos etapas de configuração e é compatível com o hardware de servidor comum. | Hosts protegidos são aprovados com base em posse da chave. | 

Outro modo chamado **atestado de Admin confiável** é substituído, começando com o Windows Server 2019. Esse modo se baseava na associação a um host protegido em um grupo de segurança de serviços de domínio Active Directory (AD DS) designado. Atestado de chaves do host fornecer a identificação de host semelhante e é mais fácil de configurar. 
...
- Uma política de segurança que determina se as VMs criadas usando os dados de blindagem estão configuradas como blindadas ou suporte para criptografia
...
A tabela a seguir resume as diferenças entre as VMs blindadas e suporte para criptografia.

| Capacidade        | Suporte à criptografia de geração 2     | Blindado de geração 2         |
|----------|--------------------|----------------|
...
|Conexão de máquina virtual (Console), dispositivos HID (por exemplo, teclado, mouse) | Ligados, não podem ser desabilitados | Habilitada em hosts, começando com o Windows Server versão 1803; Desabilitado em hosts anteriores |
|Portas seriais/COM   | Com suporte                             | Desabilitados (não podem ser habilitados) |
|Anexar um depurador (para o processo VM)<sup>1</sup>| Com suporte          | Desabilitados (não podem ser habilitados) |

<sup>1</sup> depuradores tradicionais que se conecte diretamente a um processo, como WinDbg.exe, estão bloqueadas para VMs blindadas porque o processo de trabalho da VM (VMWP.exe) é uma luz de processo protegido (PPL). Técnicas de depuração alternativas, como os usados pelo LiveKd.exe, não são bloqueadas. Ao contrário de VMs blindadas, o processo de trabalho para VMs com suporte de criptografia não é executado como um PPL depuradores tradicionais como WinDbg.exe continuarão a funcionar normalmente. 
...
2. Hosts exigem atestado.
    ...
    **Atestado de TPM confiável**: Host Hyper-V envia informações que incluem:
    ...
    **Atestado de chaves de host**: Host Hyper-V envia público metade do par de chaves. O HGS valida o host chave estiver registrada. 
    
    **Atestado de Admin confiável**: Host Hyper-V envia um tíquete Kerberos, que identifica os grupos de segurança que o host está em. O HGS valida que o host pertence a um grupo de segurança que foi configurado anteriormente pelo administrador de HGS confiável.

3. O atestado tem êxito (ou falha).

    O modo de Atestado determina quais verificações são necessários para atestar com êxito que o host está íntegro. Com o atestado de TPM confiável, o host TPM identidade, medidas de inicialização e política de integridade de código são validados. Atestado de chaves de host, somente o registro da chave de host é validado. 

4. Certificado de atestado enviado ao host.

    Supondo que o Atestado foi bem-sucedida, um certificado de integridade é enviado para o host e o host é considerado "protegido" (autorizado a executar VMs blindadas). O host usa o certificado de integridade para autorizar o Serviço de Proteção de Chave para liberar com segurança as chaves necessárias para trabalhar com VMs blindadas
...
7. A chave é retornada ao host.
    ...
    Se o certificado de integridade é válido, o KPS tenta descriptografar o segredo e retornar com segurança as chaves necessárias para ligar a VM. Observe que as chaves são criptografadas para a VBS do host protegido.
...
| Segurança com Base em virtualização (VBS) | Com base em um Hyper-V, ambiente de processamento e armazenamento que é protegido dos administradores. O Modo Seguro Virtual fornece ao sistema a capacidade de armazenar as chaves do sistema operacional que não são visíveis para um administrador do sistema operacional.|
| TPM virtual | Uma versão virtualizada de um Trusted Platform Module (TPM). Começando com o Hyper-V no Windows Server 2016, você pode fornecer um dispositivo virtual TPM 2.0 para que as máquinas virtuais podem ser criptografadas, assim como um TPM físico permite que um computador físico seja criptografado.|
