...
Você pode usar este tópico para aprender a gerenciar certificados para as comunicações Northbound e Southbound do controlador de rede ao implantar a rede definida pelo software \(SDN\) no Windows Server 2016 datacenter e usar System Center Virtual Machine Manager \(SCVMM\) como seu cliente de gerenciamento de SDN.
...
O SDN no Windows Server 2016 datacenter dá suporte aos certificados auto\-assinados e de autoridade de certificação \(AC\)assinada. Este tópico fornece instruções passo a passo para criar esses certificados e aplicá-los para proteger os canais de comunicação Northbound do controlador de rede com clientes de gerenciamento e comunicações Southbound com dispositivos de rede, como o software Load Balancer \(SLB\).
.
Ao usar a autenticação baseada em\-de certificado, você deve registrar um certificado em nós do controlador de rede que é usado das seguintes maneiras.

1. Criptografar a comunicação Northbound com protocolo SSL \(SSL\) entre os nós do controlador de rede e os clientes de gerenciamento, como o System Center Virtual Machine Manager.
...
Você pode criar e registrar um certificado auto\-assinado ou um certificado emitido por uma autoridade de certificação.
...
- O valor da caixa de texto **RestEndPoint** deve ser o nome de domínio totalmente qualificado do controlador de rede \(FQDN\) ou endereço IP. 
- O valor de **RestEndPoint** deve corresponder ao nome da entidade \(nome comum, CN\) do certificado X. 509.

### <a name="creating-a-self-signed-x509-certificate"></a>Criando um certificado X. 509 assinado por auto\-

Você pode criar um certificado X. 509 autoassinado e exportá-lo com a chave privada \(protegido com uma senha\) seguindo estas etapas para um nó de\-único e várias implantações de nó de\-do controlador de rede.

Ao criar\-certificados assinados automaticamente, você pode usar as diretrizes a seguir.
...
- Para implantações de NC de vários nós, o nome DNS que você especificar se tornará o FQDN do cluster do controlador de rede \(o host DNS um registro será criado automaticamente.\) 
...
Você pode usar o comando [New-SelfSignedCertificate](https://technet.microsoft.com/itpro/powershell/windows/pkiclient/new-selfsignedcertificate) do Windows PowerShell para criar um certificado auto\-assinado.
...
### <a name="creating-a-ca-signed-x509-certificate"></a>Criar uma autoridade de certificação\-certificado X. 509 assinado

Para criar um certificado usando uma AC, você já deve ter implantado uma infraestrutura de chave pública \(PKI\) com Active Directory serviços de certificados \(o AD CS\). 
...
>[!NOTE]
>Se a \(pessoal My – CERT: \ localmachine\my\) repositório de certificados no host Hyper\-V tiver mais de um certificado X. 509 com o nome da entidade (CN) como o nome de domínio totalmente qualificado do host \(FQDN\), verifique se o certificado que será usado pelo SDN tem uma propriedade de uso avançado de chave personalizada adicional com o OID 1.3.6.1.4.1.311.95.1.1.1. Caso contrário, a comunicação entre o controlador de rede e o host pode não funcionar.
...
1. No servidor em que o AD CS está instalado, em Gerenciador do Servidor, clique em **ferramentas**e, em seguida, clique em **autoridade de certificação**. O console de gerenciamento Microsoft da autoridade de certificação \(MMC\) é aberto. 
...
6. Expanda a seção **detalhes** e configure os itens a seguir.
   ...
   2. Verifique se **as políticas de aplicativo** incluem a **autenticação do servidor** \(1.3.6.1.5.5.7.3.1\) e a **autenticação do cliente** \(o\)do 1.3.6.1.5.5.7.3.2.
...
Depois de criar um certificado assinado por auto\-ou assinatura\-assinada, você deve exportar o certificado com a chave privada \(no formato. pfx\) e sem a chave privada \(no formato base-64. cer\) do snap-in de certificados. 
...
Você pode registrar o certificado de uma autoridade de certificação \(\)da AC. Se um certificado baseado em AC não for encontrado no computador host, o SCVMM criará um certificado autoassinado e o provisionará no computador host.
...
Quando você estiver usando auto\-certificados assinados, o SCVMM garantirá que os certificados necessários estejam presentes no repositório de autoridades de certificação raiz confiáveis do computador local. 
...
O software Load Balancer multiplexer \(MUX\) e o controlador de rede se comunicam pelo protocolo WCF, usando certificados para autenticação.
