...
* [O instalador falhar com mensagem: **_Não foi possível carregar o módulo 'Microsoft.PowerShell.LocalAccounts'._** ](#psmodulepath)

* Recebo um erro **Esta página/site não pode ser acessado** em meu navegador (selecione o tipo de implantação)
    * [Eu tenho o Windows Admin Center instalado como um aplicativo no Windows 10](#whitescreenw10)
    * [Eu tenho o Windows Admin Center instalado como um Gateway no Windows Server](#whitescreenws)
    * [Eu tenho o Windows Admin Center instalado como um Gateway em uma VM do Azure](#if-you-have-installed-windows-admin-center-in-an-azure-windows-server-vm)

* [Carregamentos de página inicial do Windows Admin Center, mas eu sou obrigado a no painel Adicionar Conexão ou não é possível conectar-se a qualquer máquina.](#winvercompat)

* [Recebo a mensagem: "Erro ao carregar o módulo. RPC: Expirado novas tentativas 'Ping'."](#winvercompat)

* [Recebo a mensagem: "Não é possível se conectar com segurança a esta página. Isso pode ocorrer porque o site usa configurações de segurança TLS desatualizadas ou unsafe."](#tls)

* [Estou tendo problemas com as ferramentas de área de trabalho remota, eventos e o PowerShell.](#websockets)

* [Estou tendo problemas para usar os recursos do Azure na borda](#azlogin)

* [Eu posso me conectar a alguns servidores, mas não para outros](#connectionissues)

* [Usando o Windows Admin Center em um **grupo de trabalho**](#workgroup)

* [Anteriormente, eu teria Windows Admin Center instalado, e agora, nada mais pode usar a mesma porta de TCP/IP](#urlacl)

* [Meu problema não está listado aqui, ou as etapas nesta página não resolver meu problema.](#filebug)
...
## <a name="im-having-issues-using-azure-features-in-edge"></a>Estou tendo problemas para usar os recursos do Azure na borda
...
* Informações gerais do problema dos [perguntas listadas abaixo](#filebug). 
...
## <a name="still-not-working-or-is-your-issue-not-captured-here-troubleshooting-common-questions"></a>Ainda não funcionando ou é o seu problema não é capturado aqui? [perguntas comuns de solução de problemas]
...
* Plataforma onde o Windows Admin Center está **instalado** (Windows 10 ou Windows Server):
    * Se instalado no servidor, o que é o Windows [versão](#winvercompat) dos **o computador que executa o navegador** para acessar a Windows Admin Center: 
