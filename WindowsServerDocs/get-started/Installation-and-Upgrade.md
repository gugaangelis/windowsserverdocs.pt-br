...
ms.locfileid: "6121475"
---
# Instalação e atualização do Windows Server

>Aplica-se a: Windows Server 2016, Windows Server 2012 R2, Windows Server 2012, Windows Server 2008 R2, Windows Server 2008

> [!IMPORTANT]
> Suporte estendido para Windows Server 2008 R2 e Windows Server 2008 termina em janeiro de 2020. [Saiba mais sobre suas opções de atualização](#upgrading-from-windows-server-2008-r2-or-windows-server-2008).
...
## Migração (recomendada para o Windows Server 2016)

A documentação do Windows Server [migração] ajuda você a migrar uma função ou um recurso de cada vez de um computador de origem que está executando o Windows Server para outro computador de destino que está executando o Windows Server, com uma versão igual ou mais recente. Para essas finalidades, a migração é definida como mover uma função ou um recurso e seus dados para um computador diferente, não atualizar o recurso no mesmo computador. Esse é o modo recomendado para mover seus dados e a carga de trabalho existente para uma versão mais recente do Windows Server. Para começar, verifique a [matriz de atualização e migração da função de servidor](https://go.microsoft.com/fwlink/?LinkId=828595) para Windows Server 2016.
...
 
>[!NOTE]
>A atualização funciona melhor em máquinas virtuais, em que os drivers de hardware específicos do OEM não são necessários para uma atualização bem-sucedida.
 
...
 
 
## Atualização para o Windows Server 2016
...
|-------------------------|---------------------------|
|Windows Server2008R2 Datacenter com SP1|Windows Server 2012 R2 Datacenter|
|Windows Server2008R2 Enterprise com SP1|Windows Server 2012 R2 Standard ou Windows Server 2012 R2 Datacenter|
|Windows Server2008R2 Standard com SP1|Windows Server 2012 R2 Standard ou Windows Server 2012 R2 Datacenter|
|Windows Web Server2008R2 com SP1|Windows Server 2012 R2 Standard|
...
|Windows Server2008R2 Standard com SP1 ou Windows Server2008R2 Enterprise com SP1|Windows Server 2012 Standard, Windows Server 2012 Datacenter|
|Windows Server2008R2 Datacenter com SP1|Windows Server 2012 Datacenter|
...
## Atualização do Windows Server 2008 R2 ou Windows Server 2008

Conforme descrito na [atualização do Windows Server 2008 e Windows Server 2008 R2](modernize-windows-server-2008.md), o suporte estendido para Windows Server 2008 R2/Windows Server 2008 termina em janeiro de 2020. Para garantir que nenhum espaço no suporte, você precisará atualizar para uma versão compatível do Windows Server ou hospedar novamente no Azure, movendo para [Especializado VMs do Windows Server 2008 R2](uploading-specialized-WS08-image-to-azure.md). Confira o [Guia de migração para o Windows Server](https://go.microsoft.com/fwlink/?linkid=872689) para obter informações e as considerações de planejar sua migração/atualização.
...
Conforme você estiver planejando sua atualização, lembre-se das diretrizes a seguir para a etapa intermediária de upgrade para o Windows Server 2012 R2.
  ...
  - Só há suporte para atualizações in-loco no mesmo idioma. Você não pode atualizar de um idioma para outro.

  - Você não pode migrar de uma instalação do Windows Server 2008 server core para o Windows Server 2012 R2 com o GUI do servidor (chamado de "Servidor com área de trabalho completa" no Windows Server). Você pode alternar a instalação de núcleo do servidor atualizado para o servidor com área de trabalho completa, mas apenas no Windows Server 2012 R2. Windows Server 2016 e posterior *não* suportam mudar do server core para área de trabalho completa, logo, certifique essa opção antes de atualizar para o Windows Server 2016.
  
Para obter mais informações, confira [versões de avaliação e opções de atualização para o Windows Server 2012](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj574204\(v=ws.11\)), que inclui detalhes de atualização de funções específicas.
