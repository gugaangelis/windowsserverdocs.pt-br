...
- **Instalação** é o conceito básico de colocar o novo sistema operacional em seu hardware. Especificamente, uma **instalação limpa** requer a exclusão do sistema operacional anterior. Para obter informações sobre a instalação do Windows Server 2016, confira [Requisitos de sistema e informações de instalação do Windows Server 2016](https://technet.microsoft.com/windows-server-docs/get-started/system-requirements--and-installation). Para obter informações sobre a instalação de outras versões do Windows Server, consulte [Instalação e Atualização do Windows Server](https://technet.microsoft.com//windowsserver/dn527667).
...
Para **Windows Server 2016 Essentials**: Você pode converter para a versão comercial completa, inserindo uma licença de volume, comercial ou chave OEM no comando **slmgr. vbs**.
...
Para Windows Server 2016 Standard, você pode converter o sistema para o Windows Server 2016 Datacenter da seguinte maneira: Em um prompt de comando elevado, determine o nome da edição atual com o comando **DISM /online /Get-CurrentEdition**. Para o Windows Server 2016 Standard será `ServerStandard`. Execute o comando **DISM /online /Get-TargetEditions** para obter a ID da edição que você pode atualizar para. Anote essa ID da edição, uma forma abreviada do nome da edição. Em seguida, execute **DISM /online /Set-Edition:\<ID da edição\> /ProductKey:-XXXXX-XXXXX-XXXXX-XXXXX /AcceptEula**, fornecendo a ID da edição de destino e sua chave de produto de varejo. O servidor será reiniciado duas vezes.
