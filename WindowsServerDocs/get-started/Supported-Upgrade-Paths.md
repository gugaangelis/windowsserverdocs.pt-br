---
title: Opções de upgrade e conversão para o Windows Server 2016
description: Explica todos os caminhos de upgrade com suporte para o Windows Server 2016.
ms.custom: na
...
ms.locfileid: "5858891"
---
# Opções de upgrade e conversão para o Windows Server 2016
...
- **Instalação** é o conceito básico de colocar o novo sistema operacional em seu hardware. Especificamente, uma **instalação limpa** requer a exclusão do sistema operacional anterior. Para obter informações sobre a instalação do Windows Server 2016, confira [Requisitos de sistema e informações de instalação do Windows Server 2016](https://technet.microsoft.com/windows-server-docs/get-started/system-requirements--and-installation). Para obter informações sobre a instalação de outras versões do Windows Server, confira [Instalação e Upgrade do Windows Server](https://technet.microsoft.com//windowsserver/dn527667).
...
- **Atualizar** significa mudar do seu sistema operacional existente para uma versão mais recente, mantendo o mesmo hardware. (Às vezes, isso é chamado de upgrade "in-loco") Por exemplo, se seu servidor estiver executando o Windows Server 2012, ou o Windows Server 2012 R2, você poderá atualizá-lo para o Windows Server 2016. É possível fazer upgrade de uma versão de avaliação do sistema operacional para uma versão comercial, de uma versão comercial mais antiga para uma versão mais nova ou, em alguns casos, de uma edição com licença de volume do sistema operacional para uma edição comercial comum.

> [!IMPORTANT]  
> A upgrade funciona melhor em máquinas virtuais, em que os drivers de hardware específicos do OEM não são necessários para uma upgrade bem-sucedida.  
...
> [!IMPORTANT]  
> Se o seu servidor usa o Agrupamento NIC, desabilite o Agrupamento NIC antes da upgrade e, depois, habilite-o novamente após a conclusão da upgrade. confira [Visão geral do Agrupamento NIC](https://technet.microsoft.com/library/hh831648(v=ws.11).aspx) para obter detalhes.

## Atualizando versões comerciais anteriores do Windows Server para Windows Server 2016
...
- Não há suporte para upgrades de arquiteturas de 32 bits para 64 bits. Todas as edições do Windows Server 2016 são apenas 64 bits.
- Não há suporte para upgrades de um idioma para outro.
...
- Não há suporte para upgrades de uma instalação anterior do Windows Server para uma cópia de avaliação do Windows Server. Versões de avaliação devem ser instaladas como uma instalação limpa.
...
## Considerações de atualização por função do servidor

Mesmo em caminhos de upgrade de versões comerciais anteriores do Windows Server 2016 com suporte, certas funções do servidor que já estão instaladas podem necessitar de preparação ou de ações adicionais para que a função continue a funcionar após a upgrade. confira os tópicos específicos da Biblioteca do TechNet para conhecer a função de cada servidor que deseja fazer upgrade quanto aos detalhes das etapas adicionais que poderiam ser necessárias.

## Conversão de uma versão de avaliação atual para uma versão comercial atual

É possível converter a versão de avaliação do Windows Server 2016 Standard para o Windows Server 2016 Standard (comercial) ou para o Datacenter (comercial). Do mesmo modo, é possível converter a versão de avaliação do Windows Server 2016 Datacenter para a versão Retail.

> [!IMPORTANT]  
> Para versões do Windows Server 2016 anteriores a 14393.0.161119-1705.RS1_REFRESH, você só pode executar essa conversão de avaliação para Retail com o Windows Server 2016 que foi instalado usando a opção de Experiência Desktop (não a opção Server Core). A partir da versão 14393.0.161119-1705. RS1_REFRESH e versões posteriores, você pode converter edições de avaliação em Retail, independentemente da opção de instalação usada.

Antes de tentar converter de uma versão de avaliação para a versão Retail, verifique se o servidor está realmente executando uma versão de avaliação. Para isso, siga um destes procedimentos:
...
Se o servidor estiver executando uma versão comercial em vez da versão de avaliação, confira a seção "Atualizando versões comerciais anteriores do Windows Server para Windows Server 2016" deste documento para obter instruções sobre a upgrade para o Windows Server 2016.

Para o **Windows Server 2016 Essentials**: é possível realizar a conversão na versão comercial completa informando uma licença por volume comercial ou chave OEM no comando **slmgr.vbs**.
...
1.  Se o servidor for um **controlador de domínio**, não será possível convertê-lo em uma versão comercial. Neste caso, instale um controlador de domínio adicional em um servidor que execute uma versão de avaliação e remova o AD DS do controlador de domínio executado na versão de avaliação. Para saber mais, confira [Fazer upgrade de controladores de domínio para o Windows Server 2012 R2 e o Windows Server 2012](https://technet.microsoft.com/library/hh994618.aspx).
2.  Leia os termos de licença.
3.  Em um prompt de comando elevado, determine o nome da edição atual com o comando **DISM /online /Get-CurrentEdition**. Anote a ID da edição (uma forma abreviada do nome da edição). Em seguida, execute **DISM /online /Set-Edition:\<ID da edição\> /ProductKey:XXXXX-XXXXX-XXXXX-XXXXX-XXXXX /AcceptEula**, fornecendo a ID da edição e uma chave do produto (Product Key) comercial. O servidor será reiniciado duas vezes.

Para a versão de avaliação do Windows Server 2016 Standard, também é possível realizar a conversão na versão comercial do Windows Server 2016 Datacenter em uma etapa usando o mesmo comando e a chave do produto (Product Key) correta.

> Para saber mais sobre o Dism.exe, confira [Opções de linha de comando do DISM](https://go.microsoft.com/fwlink/?LinkId=192466).

## Conversão de uma edição comercial atual em outra edição comercial atual
...
Para Windows Server 2016 Standard, você pode converter o sistema para o Windows Server 2016 Datacenter da seguinte maneira: em um prompt de comando com privilégios elevados, determine o nome da edição atual com o comando **DISM /online /Get-CurrentEdition**. Para Windows Server 2016 Standard será `ServerStandard`. Execute o comando **DISM /online /Get-TargetEditions** para obter a ID da edição, você pode atualizar para. Anote esse ID da edição, uma forma abreviada do nome da edição. Em seguida, execute **DISM /online /Set-Edition: \ < \<ID da edição > /ProductKey:-XXXXX-XXXXX-XXXXX-XXXXX /AcceptEula**, fornecendo a ID da edição de seu destino e sua chave de produto de varejo. O servidor será reiniciado duas vezes.

## Conversão de uma versão comercial atual em uma versão licenciada por volume atual
