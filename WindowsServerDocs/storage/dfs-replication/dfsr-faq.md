...
Para obter informações sobre namespaces do DFS, consulte [namespaces do DFS: perguntas](https://technet.microsoft.com/library/ee404780)frequentes.

Para obter informações sobre as novidades do Replicação do DFS, consulte os seguintes tópicos:

  - [Namespaces DFS e visão geral de replicação do DFS](https://technet.microsoft.com/library/jj127250) (no Windows Server 2012)  
...
Os aplicativos antivírus podem causar uma replicação excessiva se suas atividades de verificação alteram os arquivos em uma pasta replicada. Para obter mais informações, [teste a interoperabilidade de aplicativos antivírus com o replicação do DFS](https://go.microsoft.com/fwlink/?linkid=73990) (https://go.microsoft.com/fwlink/?LinkId=73990).
...
Sim. No entanto, você deve primeiro instalar o Windows Server 2003 Service Pack 2 ou o hotfix. Para obter mais informações, consulte o [artigo 920335](https://go.microsoft.com/fwlink/?linkid=76776) na base de dados de conhecimento Microsoft (https://go.microsoft.com/fwlink/?LinkId=76776). Além disso, a replicação de um volume inteiro pode causar os seguintes problemas:
...
Para obter mais informações, consulte "requisitos e delegação de segurança do Replicação do DFS" no [delegar a capacidade de gerenciar replicação do DFS](https://go.microsoft.com/fwlink/?linkid=182294) (https://go.microsoft.com/fwlink/?LinkId=182294).
...
Para atualizar ou substituir um membro Replicação do DFS, consulte esta postagem de blog no blog Ask the Directory Services Team: [substituindo o hardware ou o sistema operacional do membro do DFSR](https://blogs.technet.com/b/askds/archive/2010/09/10/series-wrap-up-and-downloads-replacing-dfsr-member-hardware-or-os.aspx).
...
Sim. Certos cenários têm suporte ao replicar perfis de usuário móvel. Para obter informações sobre os cenários com suporte, consulte a [declaração de suporte da Microsoft sobre os dados de perfil de usuário replicados](https://go.microsoft.com/fwlink/?linkid=201282) (https://go.microsoft.com/fwlink/?LinkId=201282).
...
Para obter uma lista das diretrizes de escalabilidade que foram testadas pela Microsoft para o Windows Server 2003 R2, consulte [diretrizes de escalabilidade replicação do DFS](https://go.microsoft.com/fwlink/?linkid=75043) (https://go.microsoft.com/fwlink/?LinkId=75043).
...
Sim. Para obter mais informações, consulte o [pacote de gerenciamento do replicação do DFS para System Center Operations Manager 2007](https://go.microsoft.com/fwlink/?linkid=182265) no centro de download da Microsoft (https://go.microsoft.com/fwlink/?LinkId=182265).
...
> [!IMPORTANT]
> Para exibir ou gerenciar grupos de replicação que contêm pastas replicadas somente leitura ou membros que são clusters de failover, você deve usar a versão do Gerenciamento DFS incluída no Windows Server 2012 R2, Windows Server 2012, Windows Server 2008 R2, o <a href="https://go.microsoft.com/fwlink/p/?linkid=238560">ferramentas de administração de servidor remoto para Windows 8</a>ou o <a href="https://technet.microsoft.com/library/ee449475">ferramentas de administração de servidor remoto para Windows 7</a>. 
<br>
...
Para saber mais sobre os diferentes métodos de ajuste do desempenho de replicação, consulte [ajustando o desempenho de replicação no DFSR](https://blogs.technet.com/b/askds/archive/2010/03/31/tuning-replication-performance-in-dfsr-especially-on-win2008-r2.aspx) no [blog da equipe de serviços de diretório](https://blogs.technet.com/b/askds/).
...
Para obter uma lista de valores de atributo e suas descrições, consulte [atributos de arquivo](https://go.microsoft.com/fwlink/?linkid=182268) no MSDN (https://go.microsoft.com/fwlink/?LinkId=182268).

Os valores de atributo a seguir são definidos usando a função `SetFileAttributes dwFileAttributes` e são replicados por Replicação do DFS. As alterações nesses valores de atributo disparam a replicação dos atributos. O conteúdo do arquivo não é replicado, a menos que o conteúdo também seja alterado. Para obter mais informações, consulte a [função SetFileAttributes](https://go.microsoft.com/fwlink/?linkid=182269) na biblioteca MSDN (https://go.microsoft.com/fwlink/?LinkId=182269).

  - ATRIBUTO de\_de arquivo\_oculto  
      
  - ATRIBUTO de\_de arquivo\_READONLY  
      
  - SISTEMA de\_de atributo de\_de arquivos  
      
  - O atributo de\_de arquivo não\_\_conteúdo\_INDEXado  
      
  - ATRIBUTO de\_de arquivo\_OFFLINE  
      

Os valores de atributo a seguir são replicados por Replicação do DFS, mas não disparam a replicação.

  - Arquivo de\_do atributo de\_de arquivos  
      
  - ATRIBUTO de\_de arquivo\_NORMAL  
      

Os seguintes valores de atributo de arquivo também disparam a replicação, embora não possam ser definidos usando a função `SetFileAttributes` (use a função `GetFileAttributes` para exibir os valores de atributo).

  - ATRIBUTO de\_de arquivo\_ponto de\_de nova análise  
      

> [!NOTE]
> Replicação do DFS não replica valores de atributo de ponto de nova análise, a menos que a marca de nova análise seja IO_REPARSE_TAG_SYMLINK. Arquivos com as IO_REPARSE_TAG_DEDUP, IO_REPARSE_TAG_SIS ou IO_REPARSE_TAG_HSM marcas de nova análise são replicadas como arquivos normais. No entanto, a marca de nova análise e os buffers de dados de nova análise não são replicados para outros servidores porque o ponto de nova análise funciona apenas no sistema local. 
<br>

  - ATRIBUTO de\_de arquivo\_COMPACTado  
      
  - ATRIBUTO de\_de arquivo\_CRIPTOGRAFAdo  
      

> [!NOTE]
> Replicação do DFS não replica os arquivos que são criptografados usando o Encrypting File System (EFS). O Replicação do DFS replica arquivos que são criptografados usando um software que não seja da Microsoft, mas somente se ele não definir o valor do atributo FILE_ATTRIBUTE_ENCRYPTED no arquivo. 
<br>

  - ATRIBUTO de\_de arquivo\_arquivo de\_ESPARSO  
      
  - DIRETÓRIO de\_do atributo de\_de arquivo  
      

Replicação do DFS não replica o atributo de\_de arquivo\_valor temporário.
...
A replicação inicial não precisa replicar o conteúdo quando os arquivos diferem somente por atributos reais ou carimbos de data/hora. Um atributo real é um atributo que pode ser definido pela função do Win32 `SetFileAttributes`. Para obter mais informações, consulte a [função SetFileAttributes](https://go.microsoft.com/fwlink/?linkid=182269) na biblioteca MSDN (https://go.microsoft.com/fwlink/?LinkId=182269). Se dois arquivos diferirem por outros atributos, como compactação, o conteúdo do arquivo será replicado.
...
Replicação do DFS não mescla arquivos quando há um conflito. No entanto, ele tenta preservar a versão mais antiga do arquivo na pasta Hidden DfsrPrivate\\ConflictandDeleted no computador em que o conflito foi detectado.
...
Para obter mais informações, consulte os seguintes sites da Microsoft:
  ...
  - [Constantes de nível de autenticação](https://go.microsoft.com/fwlink/?linkid=182280)  
