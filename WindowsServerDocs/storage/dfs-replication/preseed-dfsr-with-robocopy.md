---
title: Usar o Robocopy para pré-propagar arquivos para a Replicação do DFS
description: Como usar o Robocopy.exe para pré-propagar arquivos para Replicação do DFS.
ms.topic: article
author: JasonGerend
ms.author: jgerend
ms.date: 05/18/2018
...
>[!IMPORTANT]
>O Robocopy não copia arquivos exclusivamente bloqueados. Se os usuários tendem a bloquear muitos arquivos por longos períodos em seus servidores de arquivos, considere usar um método de pré-propagação diferente. A pré-propagação não exige uma correspondência perfeita entre as listas de arquivos nos servidores de origem e de destino, porém, quanto mais houver arquivos que não existam quando a sincronização inicial for executada para Replicação do DFS, menos eficaz será a pré-propagação. Para minimizar conflitos de bloqueio, use o Robocopy fora do horário de pico para sua organização. Sempre examine os logs do Robocopy após a pré-propagação para garantir que você entendeu quais arquivos foram ignorados devido a bloqueios exclusivos.

Para usar o Robocopy para pré-propagar arquivos para a Replicação do DFS, siga estas etapas:
...
Como a pré-propagação não envolve diretamente a Replicação do DFS, você só precisa cumprir os requisitos para executar uma cópia de arquivo com o Robocopy.
...
Depois de minimizar os bloqueios nos arquivos que serão replicados, você pode pré-propagar os arquivos do servidor de origem para o servidor de destino.
...
### <a name="pre-seed-the-replicated-files-onto-the-destination-server-with-robocopy"></a>Pré-propagar os arquivos replicados para o servidor de destino com o Robocopy
