...
**Grava:** ReFS serviços as gravações recebidas de três formas diferentes:
...
2. **Grava o espelho, realocado de paridade:**

    Se a gravação de entrada modificar os dados que estiverem na paridade e o ReFS encontrar com êxito espaço livre suficiente em espelho para atender a gravação de entrada, o ReFS primeiro invalidará os dados anteriores em paridade e, em seguida, registrará no espelho. Essa invalidação é uma operação de metadados rápida e barata que ajuda a aumentar significativamente o desempenho de gravação feito a paridade.
...
**Grava:** Cada tipo de gravação descrito acima tem suas próprias características de desempenho. Em termos gerais, gravações para o nível de espelho são muito mais rápidas que as gravações realocadas, e estas são significativamente mais rápidas que gravações feitas diretamente para a faixa de paridade. Essa relação é ilustrada com a desigualdade abaixo: 


- **Espelhar camada > realocadas gravações >> camada de paridade**


**Leituras:** Não há nenhum impacto de desempenho negativo significativo durante a leitura de paridade:
...
>[!IMPORTANT]
> É recomendável colocar VHDs pesada de gravação em subdiretórios diferentes. Isso ocorre porque o ReFS grava as alterações de metadados no nível de um diretório e seus arquivos. Portanto, se você distribuir arquivos de gravação intensa entre diretórios, operações de metadados são menores e executados em paralelo, reduzindo a latência para aplicativos.
...
### <a name="increasing-the-size-of-the-mirrored-tier"></a>Aumentar o tamanho da camada de espelho
