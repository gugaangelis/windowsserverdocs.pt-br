---
title: Implantar a impressão de nuvem híbrida do Windows Server
description: Como configurar a impressão da nuvem híbrida Microsoft
...
- Etapa 3: Instalar o Azure Proxy de aplicativo (AAP) com o Kerberos Constrained Delegation (KCD)
...
### <a name="step-3---install-azure-application-proxy-aap-with-kerberos-constrained-delegation-kcd"></a>Etapa 3: instalar o Proxy de aplicativo do Azure (AAP) com o Kerberos Constrained Delegation (KCD)
...
3. No computador do Active Directory, vá para **Ferramentas -> usuários e computadores**
    - Selecione o computador que está executando o conector de Proxy de aplicativo
    - Clique com botão direito e selecione **Propriedades -> delegação** guia
    ...
    - Sob **serviços aos quais esta conta pode apresentar credenciais delegadas**
        - Adicione o valor da identidade SPN do computador executando os serviços (serviço MopriaDiscoveryService e MicrosoftEnterpriseCloudPrint)
            - Para o SPN, insira o SPN da máquina em si, ou seja "HOST /\<MachineName\>.\< Domínio\>"<br>
                `HOST/appServer.Contoso.com`
4. Volte para o portal de gerenciamento de locatário do AAD e adicione os proxies de aplicativo
    ...
    - Depois de criado, vá até **aplicativos empresariais** -> **todos os aplicativos**, selecione o novo aplicativo que você acabou de criar
    - Vá para **logon único**, verifique se o "único modo de logon" é definido como "Autenticação integrada do Windows"
    - Defina o "SPN do aplicativo interno" para o SPN que você especificou na etapa 3.3 acima
    - Verifique se que o "identidade de logon delegada" é definida como "Nome UPN do usuário"

5. Repita 4, acima, para o serviço de impressão do Enterprise Cloud e forneça a URL interna ao serviço de impressão de nuvem empresarial
6. Volte para o portal de gerenciamento de locatário do Azure AD e vá para **registros de aplicativo** e selecione o aplicativo cliente nativo -> "Configurações"
    - Selecione **permissões necessárias**
        - Adicione os 2 novos aplicativos de proxy que acabou de criar
        - Conceder permissões delegadas para esses 2 aplicativos
        - Quando ambos os aplicativos de proxy foram adicionados, clique em "Conceder permissões".  Selecione "Sim" quando for solicitado a aprovar a solicitação.

7. Habilitar a autenticação do Windows no IIS para os computadores do serviço de nuvem do Mopria e o serviço de impressão do Enterprise Cloud
    - Verifique se o recurso de autenticação do Windows está instalado:
        ...
        4. Selecione **instalação baseada em função ou recurso**
        5. Selecione o servidor
        6. No servidor Web (IIS) -> servidor Web -> segurança, selecione **autenticação do Windows**
        7. Clique em Avançar até que você conclua a instalação
    - Habilite a autenticação do Windows no IIS:
        ...
        2. Para cada serviço/site:
            ...
            3.  Clique em **autenticação do Windows** e clique em **habilitar** sob **ações**
...
- Localize o grupo de políticas do Enterprise Cloud Print e configurar as políticas a seguir as diretrizes abaixo:
    ...
    - MopriaDiscoveryResourceId = URL externa do Mopria descoberta de serviço do Azure do Proxy de aplicativo criado na etapa 3.4 (deve ser exatamente o mesmo incluindo à direita /)
    - CloudPrintResourceId = URL externa do Enterprise Cloud Print serviço Azure Proxy do aplicativo criado na etapa 3.5 (deve ser exatamente o mesmo incluindo à direita /)
...
- OMA-URI
    ...
    - `MopriaDiscoveryResourceId = ./Vendor/MSFT/Policy/Config/EnterpriseCloudPrint/MopriaDiscoveryResourceId`
        - Valor = URL externa do Mopria descoberta de serviço do Azure do Proxy de aplicativo criado na etapa 3.4 (deve ser exatamente o mesmo incluindo à direita /)
    - `CloudPrintResourceId = ./Vendor/MSFT/Policy/Config/EnterpriseCloudPrint/CloudPrintResourceId`
        - Valor = URL externa do Enterprise Cloud Print serviço Azure Proxy do aplicativo criado na etapa 3.5 (deve ser exatamente o mesmo incluindo à direita /)
...
5. Em um computador de atualização do criador de outono do Windows 10, abra um prompt de comando com privilégios elevados do Windows PowerShell
    1. Execute os seguintes comandos
        ...
        - Publish-CloudPrinter
            ...
            - DiscoveryEndpoint = URL externa do Mopria descoberta de serviço do Azure do Proxy de aplicativo criado na etapa 3.4
            - PrintServerEndpoint = URL externa do Enterprise Cloud Print serviço Azure Proxy do aplicativo criado na etapa 3.5
...
- Abra um navegador da web e ir para https://&lt;serviços de máquina-ponto de extremidade &gt; /mcs/services (a URL externa para o ponto de extremidade de descoberta)
