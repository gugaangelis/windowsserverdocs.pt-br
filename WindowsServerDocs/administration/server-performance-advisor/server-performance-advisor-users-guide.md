...
* Gerenciar o desempenho do servidor e solucionar problemas de desempenho do servidor.

* Fornece relatórios de dados e recomendações sobre problemas de desempenho e de configuração comuns.

* Fornece melhores recomendações pratice com base nos dados coletados.
...
A versão original do Microsoft Server Performance Advisor (SPA) foi lançada em 2005-2006. Ele destina, principalmente, Windows Server 2003, incluindo o sistema operacional principal e funções de servidor, como serviços de informações da Internet (IIS) e o active directory. O objetivo da ferramenta é para ajudá-lo a avaliar o desempenho do servidor e solucionar problemas de desempenho do servidor.
...
### <a href="" id="what-s-new-in-spa-3-1-"></a>O que é novo no SPA 3.1?

Os seguintes recursos e aprimoramentos foram adicionados no SPA 3.1:

* Pacote de Supervisor de Active directory ajuda a analisar o desempenho geral da função de servidor dos serviços de domínio do Active Directory do servidor s.
...
* **Ambiente de servidor**
    ...
    SPA é projetado para configurar e manter facilmente. Aplica-se aos ambientes de servidor que usam servidores de 1 a 100. SPA não se adapta bem se você está tentando gerenciar o desempenho de mais de 100 servidores. Para ambientes maiores ou mais sofisticados, considere usar o System Center Operations Manager.
...
* **Solucionar problemas de desempenho**
    ...
    SPA gera um relatório que você pode exibir. SPA relatórios fornecem uma lista de notificação que realça os potenciais problemas e uma seção de dados que inclui várias configurações para o servidor, configurações e o índice de desempenho. Você pode usar esse relatório para identificar o problema de desempenho específicos e usar as recomendações para encontrar soluções para o problema. Relatórios podem ser comparados com outros relatórios que foram gerados em um momento diferente ou por um servidor diferente. Usando esta comparação lado a lado, você pode determinar as diferenças entre a linha de base normal versus um comportamento anormal.

    **Observação** SPA não foi projetado para ser uma depuração ou a ferramenta de medição. Além disso, da perspectiva dos servidores, ela pode ser considerada uma ferramenta de somente leitura e não modifica as configurações de servidores.
...
Em um aplicativo de negócios típico, existem três camadas: a camada de apresentação, a camada de lógica comercial e a camada de armazenamento. SPA é projetado como uma de duas camadas produto o console e o banco de dados. O console serve como uma camada de apresentação com determinados lógica relacionados ao processo incluída e serve o banco de dados como a camada de armazenamento e a camada de lógica de negócios. O console de entrada do usuário de captura e controla as etapas para a coleta de dados, processamento de dados e geração de relatórios. SPA não depende de serviços de sistema do Windows.
...
**Observação** SPA não inclui SQL Server ou o .NET Framework como parte do pacote de instalação do SPA. Depois de instalar o Microsoft SQL Server 2008 R2 e o .NET Framework 4.0, é recomendável que você execute o Windows Update antes de instalar o SPA.
...
### <a href="" id="bkmk-setupspa"></a>Configurando SPA

SPA é empacotado como um arquivo. cab que inclui todos os binários para o framework do SPA, os cmdlets do Windows PowerShell que são usados em cenários avançados, e o Supervisor seguinte pacotes: Principais do sistema operacional, o Hyper-V, o active directory e o IIS. Após extrair o arquivo. cab para uma pasta, nenhuma instalação adicional é necessária. No entanto, para executar o SPA, você precisa habilitar a coleta de dados de servidores de destino da seguinte maneira:
...
* Certifique-se de que a conta de usuário usada para executar o console do SPA leu e permissões de gravação para a pasta compartilhada. CCP usa essa conta para gravar logs na pasta.
O console do SPA usa a mesma conta para ler os logs e importá-los no banco de dados.

    **Observação** ETW implementa um buffer circular para armazenar o rastreamento e os move para a pasta compartilhada, quando possível. Se o servidor está ocupado ou a operação de gravação estiver lenta, o ETW descarta os rastreamentos quando o buffer estiver cheio. Isso é importante que a pasta compartilhada está localizada em um servidor com acesso de e/s rápido. É recomendável que cada servidor de destino tem uma pasta compartilhada para minimizar a perda de dados causada por e/s de arquivo lenta.
...
* Se o servidor de console e de destino está localizado em sub-redes diferentes, você também precisa definir o campo de endereço IP remoto nas regras de firewall de entrada na **escopo** configurações sobre o **alertas e Logs de desempenho** página, conforme mostrado aqui.
...
* Se o servidor de destino não tiver ingressado em um domínio, habilite a seguinte configuração de registro: **HKLM\\SOFTWARE\\Microsoft\\Windows\\Currentversion\\Policies\\system\\LocalAccountTokenFilterPolicy**.

**Observação** por padrão, o SPA grava logs de diagnóstico para a pasta onde se encontra SpaConsole.exe. Se o SPA é instalado na pasta arquivos de programas, SPA só é ser capaz de gravar no log quando SpaConsole.exe é executado como um administrador.

Se você quiser executar a análise de dados no computador do console, você precisa executar o SPA como administrador. CCP passa por um caminho de código diferente ao executar em um computador local, o que requer privilégios de administrador.

Se você quiser executar o IIS SPA Advisor Pack em seus servidores, você precisa habilitar o rastreamento ETW para o servidor IIS e consulta do WMI. Você pode fazer isso habilitando **rastreamento** sob o **integridade e diagnóstico** serviço de função e **Ferramentas e Scripts de gerenciamento do IIS** em **Ferramentas de gerenciamento** do **servidor Web (IIS)** função de servidor.

 

### <a name="creating-your-first-project"></a>Criando seu primeiro projeto

Depois que tudo estiver configurado, você pode criar seu primeiro projeto SPA. Conforme descrito na seção anterior, o SPA armazena tudo que estiver relacionado à análise de dados de desempenho dentro de um banco de dados. Cada projeto SPA corresponde a um único banco de dados. O **primeira vez Use assistente** orienta você pelas etapas a seguir.
...
2.  Para criar um novo projeto, clique em **arquivo**, e, em seguida, clique em **novo projeto**. Isso inicia na primeira vez, Use o assistente. A primeira página mostra as etapas a seguir ao usar o Assistente:
...
3.  Clique em **Avançar**. O **criar banco de dados do projeto** página solicita que você forneça o nome da instância do Microsoft SQL Server onde você deseja criar seu banco de dados. Por exemplo, se ele estiver no mesmo computador que o console, você pode usar **localhost\\&lt;o nome do servidor SQL&gt;**.

    **Observação** o nome da instância padrão para uma instalação do SQL Server 2008 R2 Express é SQLExpress. Para uma instância do SQL Server 2008 R2 Express instalado no computador local, o banco de dados normalmente por padrão para **localhost\\SQLExpress**. No entanto, ele pode foram alterado durante a instalação do SQL Server, então você precisa certificar-se de que você use o nome da instância do SQL Server à direita.
...
4.  Forneça o nome do banco de dados. Somente letras, dígitos e sublinhados (\_) são permitidos como caracteres válidos para um nome de banco de dados. Uma sugestão razoável para o nome de banco de dados do SPA seria **SPA**. Se você inserir um nome inválido, um ícone vermelho de erro é exibida. A dica de ferramenta associada fornece o motivo da falha de validação.
    ...
    **Observação** é importante lembrar-se o nome do banco de dados e o nome da instância de servidor, pois esses são apenas identificadores para o seu projeto. Você precisa fornecer essas informações se você desejar alternar para esse banco de dados.

     

5.  Depois de fornecer o nome da instância de servidor e o nome do banco de dados, na primeira vez que Use o assistente gera o local para o arquivo de banco de dados.

6.  Sobre o **criar banco de dados do projeto** , clique em **próxima**. Na primeira vez, Use o assistente cria um banco de dados e gera todos os procedimentos armazenados, funções e esquema de banco de dados relacionados ao SPA no banco de dados. Esta etapa pode levar vários segundos dependendo da velocidade de hardware e de rede.

    **Observação** se isso falhar, uma mensagem de erro é exibida. Alguns dos problemas mais comuns são: Console não pode se conectar à instância do SQL Server, privilégios insuficientes para criar o banco de dados, ou o nome do banco de dados já existe.

     

7.  Quando a etapa anterior for bem-sucedido, você vê o **provisionar pacote advisor** página. Ela lista todos os pacotes de supervisor que estão disponíveis em seu computador. SPA verifica automaticamente a pasta denominada **APs** sob o diretório raiz do SPA. Ele lista o nome completo, versão e autor de cada pacote de Supervisor.

    **Observação** para obter mais informações sobre como o nome completo e a versão são usadas em SPA, consulte [Gerenciando pacotes advisor](#bkmk-manageadvisorpacks)
...
8.  Escolha quais pacotes de supervisor que deseja provisionar o banco de dados do projeto e, em seguida, clique em **próxima**. Ou você pode clicar em **Ignorar** para mover para a próxima etapa sem o provisionamento de todos os pacotes de Supervisor.
    ...
    **Observação** você pode provisionar pacotes advisor a qualquer momento você estiver usando a ferramenta. Para obter mais informações, consulte [Gerenciando pacotes advisor](#bkmk-manageadvisorpacks).

     

9.  Sobre o **adicionar servidores** página, para cada servidor a ser adicionado à lista de servidores de destino, há dois campos obrigatórios para preencher: **Nome do servidor** e **local de compartilhamento de arquivo**.

    **Observação** também há uma **comentário** campo, que é usado principalmente para classificar ou localizar o servidor. Em casos onde você tem vários servidores, você pode importar um arquivo de valor (. csv) separada por vírgulas que contém o nome do servidor, a pasta de resultado e o campo de comentário opcional. O **comentário** campo é usado para descrever o servidor e o termo pode ser usado para filtrar os servidores para coleta de dados. Se você estiver inicializando os servidores por meio do arquivo. csv, um erro de análise dentro do arquivo não carrega os servidores.

     

10. Várias configurações precisam ser definidas para habilitar a coleta de dados do CCP, conforme descrito em [Configurando SPA](#bkmk-setupspa). O **Adicionar servidor** página fornece uma funcionalidade de configuração de teste para ajudar você a solucionar problemas de configuração. Marque a caixa de seleção associada ao computador e, em seguida, clique em **testar a conectividade**. SPA tenta gerar um coletor de dados definidas em servidores de destino, e ele tenta importar que os resultados de volta para o banco de dados. Se tudo estiver correto, o **Status** mostra **passar**. Se ele falhar, será exibida uma dica de ferramenta que descreve o motivo da falha.

11. Cada um dos servidores é adicionada automaticamente ao banco de dados, mesmo se ele não passar no teste de configuração. Para remover servidores da lista, selecione o nome do servidor e clique em **remover**.

12. Quando tudo o que for concluído, clique em **concluir** para fechar a primeira vez, Use o assistente. Se você fechar na primeira vez Use assistente antes da conclusão, manter todas as etapas anteriores e nenhum deles reverter automaticamente. Você precisa fazer alterações futuras manualmente.
...
* **Resultado da análise** por padrão, ele mostra o resultado da análise de desempenho mais recente execução no servidor. Se não houve alguma análise de desempenho executado no servidor, ele mostra **Nenhum relatório**. Se houver um aviso gerado pelo relatório, ele mostra **Aviso** e o carimbo de hora quando o relatório mais recente foi gerado. Se nenhum problema foi encontrado durante a análise mais recente no servidor, ele mostra **OK** e o carimbo de hora.
    ...
    **Observação** se você alterou recentemente uma configuração do sistema, é recomendável que você execute a análise novamente para avaliar o impacto geral da alteração e obter um relatório atualizado do estado do sistema. SPA não controla as alterações de configuração para o sistema em teste.
...
* **comentário de** descreve o servidor de destino atual. Por exemplo, você pode descrever seu servidor usando a função de servidor (por exemplo, o SQL Server) ou um local (por exemplo, Kent). SPA usa o **nome do servidor** e **comentário** para ajudar a pesquisar e localizar o servidor apropriado. Você pode digitar na caixa de texto de pesquisa. Se o **nome do servidor** ou **comentário** colunas contém a cadeia de caracteres exata que você inseriu na caixa de pesquisa, o servidor é exibido na lista de servidores.
...
* **Remover recorrência** um botão que permite cancelar um contínuo repetir o trabalho de coleção. Cancela a coleção de repetição, mas não a coleção atual (se houver) está em andamento. Esta opção permite que você redefinir um novo intervalo de repetição de coleção ou execute a coleção manualmente.
...
Para executar a análise de desempenho em servidores de destino, selecione os servidores que você deseja executar a análise de desempenho em relação a e, em seguida, clique em * * executar a análise de * * uma caixa de diálogo é exibida e solicita que você selecione os pacotes de Supervisor para ser executado em servidores. Os pacotes selecionados advisor executados simultaneamente. Os relatórios são gerados se baseiam os dados de desempenho coletados durante o mesmo período de tempo.

**Observação** se você selecionar um servidor que tenha uma análise de desempenho recorrente em execução, o **Remover recorrência** botão permite que você cancele a coleta de dados recorrente. SPA não permite várias sessões de coleta de dados ao mesmo tempo no mesmo computador.
...
Em SPA, há três tipos de relatório de análise de desempenho: Único relatório, relatório de lado a lado e tendências e gráficos históricos.
...
Também é possível pesquisar dentro de um relatório. Um número de relatórios têm uma **localizar** caixa de pesquisa de cadeia de caracteres disponível para pesquisa de cadeia de caracteres de texto rápido dentro do relatório. Para remover a caixa de texto, você pode descartá-la. Para ativar uma caixa de pesquisa (no windows com o texto de pesquisa), você pode usar CTRL + F atalho. O **encontrar** caixa permite que o usuário especifique uma pesquisa diferencia maiusculas de minúsculas, conforme apropriado, com o **diferenciar maiusculas de minúsculas** opção.

A figura a seguir mostra a **encontrar** caixa de pesquisa com a cadeia de caracteres **Power** sobre o **relatório** guia.
...
Se a fonte de dados para uma regra atenderem a determinadas condições com base nas configurações de limite e de lógica, um aviso é exibido na **aviso** área. Um aviso inclui as seguintes partes:
...
* **Valor único** um par chave/valor. A chave é uma cadeia de caracteres que representa o significado do valor. O valor pode ser uma cadeia de caracteres, um valor numérico ou um valor booleano. Isso geralmente é usado para mostrar informações estáticas, como arquitetura de configuração, por exemplo, a CPU, o tamanho total da memória e a versão do BIOS, que não são alterados ao longo do tempo.

* **Liste um valor** isso às vezes é um par chave/valor, mas o valor da lista pode conter vários campos. Por exemplo, o atributo da CPU pode ser mostrado em uma tabela com várias colunas e várias linhas. Cada linha representa uma CPU, e cada coluna representa um atributo da CPU.
...
### <a href="" id="side-by-side-report-"></a>Relatório de lado a lado
...
Se você ignorar a **localizar** caixa de diálogo, você pode reativá-la digitando CTRL + F. Essa caixa de diálogo localiza e realça as cadeias de caracteres de texto dentro da seção atual.

Na seção de notificação, se qualquer um dos resultados dos dois relatórios que são comparados é um aviso, ele é listado na **aviso** área. Caso contrário, os resultados são listados na **outras notificações** área. Como a chave para um relatório de lado a lado é identificar as diferenças entre relatórios, não há informações detalhadas sobre uma regra são exibidas. Os usuários podem clicar para abrir o formulário de detalhes da regra para obter mais informações sobre a regra o nome da regra.
...
1.  Para ajudar a localizar anormalidades em um determinado momento para um ponto de dados por exemplo, às 2 horas por dia, o **latência média de solicitação** do IIS vai de cerca de 200 ms para 500 ms.
...
A série de dados selecionada em tendências e históricos gráficos é armazenada como uma preferência de usuário. Na próxima vez que o gráfico de histórico e tendência Visualizador é aberto para o mesmo pacote de Supervisor, o mesmo conjunto de séries de dados são listados como padrão.
...
1.  Copiar todo o conteúdo do pacote de Supervisor sob o *% SpaRoot %*\\directory APs.
...
2.  Na janela principal, clique em **configuração**, e, em seguida, clique em **configurar pacotes Advisor**. O **configurar pacotes Advisor** caixa de diálogo é aberta.
    ...
    **Observação** nessa caixa de diálogo é semelhante de **provisionar pacote advisor** página pela primeira vez Use o assistente. Ele mostra uma lista de pacotes de supervisor que estão disponíveis para gerenciar. Cada pacote de Supervisor na lista tem propriedades, como nome, instalado a versão, a versão e autor. Nome é o nome completo do pacote de Supervisor e a versão instalada é a versão deste pacote de advisor já foi provisionado no projeto. Se o pacote de Supervisor não está provisionado no banco de dados atual, a caixa de texto da versão instalada exibe **não instalado**. O campo versão indica a versão deste pacote de Supervisor, sob a pasta de pacotes do Supervisor é arquivado.

     

3.  Selecione o pacote de Supervisor na lista. Se o pacote de Supervisor não foi provisionado ou se não houver uma versão mais recente na pasta de pacotes advisor daquele banco de dados, o **provisionar** botão está habilitado. Clique o **provisionar** botão.

4.  Quando o provisionamento for concluído, o **versão instalada** campo para o pacote selecionado advisor contém as novas informações de versão.
...
Para obter mais informações sobre como alterar e criar pacotes do advisor, consulte [Server Performance Advisor Pack Development Guide](server-performance-advisor-pack-development-guide.md).
...
1.  No **configuração** menu, clique em **configurar pacotes de Supervisor**, clique no nome do pacote a ser modificada e, em seguida, clique em Supervisor de **Configurar**.
    ...
    **Observação** você verá uma lista de todas as regras que estão incluídos no pacote do Supervisor. A caixa de seleção à esquerda do nome do pacote advisor indica se a regra está habilitada. Se uma regra for desactivada, ele ficará oculta de todos os relatórios.
...
Algumas das regras não têm limites definidos. Por exemplo, o **HTTP Keep Alive** regra verifica uma configuração booleana para IIS. Portanto, a lista de limites pode ser vazia. Caso contrário, todos os limites que são usados pela regra atual é listado. Uma descrição detalhada sobre como um limite é usado na regra é incluída como parte da descrição.

Se o **detalhes de regra** formulário é iniciado a partir de **configuração** menu, a lista de limite tem três colunas: nome, a configuração original e a configuração de alteração. Se ele é iniciado em um relatório único ou lado a lado, os valores de limite são usados pelo relatório também são ser incluídos. Os usuários podem modificar os valores de limite atuais, alterando o valor em **alterar a configuração** coluna e, em seguida, clicando em **salvar** para salvar as alterações no banco de dados.

Todas as alterações feitas nos limites é apenas ser aplicada aos relatórios gerados após as alterações. Os relatórios existentes não são afetados por essas alterações.

### <a name="sharing-thresholds"></a>Compartilhamento de limites

Se você gerenciar seus servidores em situações semelhantes, você pode optar por usar o mesmo conjunto de limites. Você pode exportar e importar os limites de um pacote específico advisor usando o **configuração** menu. Selecione o pacote de Supervisor específico e, em seguida, clique em **Configurar**. O arquivo exportado de limite está em um formato XML.

Ao importar um limite, o SPA valida o formato de arquivo XML e verifica se o arquivo corresponde o pacote selecionado advisor. Se for bem-sucedida, SPA importa todos os valores do arquivo de limite para o banco de dados do projeto atual. Semelhante ao cenário de limites de alteração anterior, todas as alterações de valor de limite somente entram em vigor nos relatórios que são gerados no futuro. Os relatórios existentes não são afetados.

### <a name="enable-or-disable-rules"></a>Habilitar ou desabilitar regras

Uma regra pode ser habilitada ou desabilitada da **regra detalhes** formulário. Você precisa clicar **Salvar** para manter as alterações feitas. Se uma regra é desabilitada, não é a ser exibido em qualquer um dos relatórios. Mas a lógica de negócios é disparada ao gerar o relatório, portanto, quando você optar por reabilitar a regra, ele aparece em relatórios novamente.

### <a name="reset-advisor-packs-to-original-state"></a>Redefinir os pacotes do advisor ao estado original
...
Para obter mais informações sobre como alterar um pacote provisionado advisor, consulte o [Server Performance Advisor Pack Development Guide](server-performance-advisor-pack-development-guide.md).

Se você quiser reverter as alterações que foram feitas em um pacote de Supervisor provisionado, você pode optar por redefinir o pacote de Supervisor. Isso substituirá todos os scripts SQL que estão relacionados ao pacote do Supervisor e redefinir todos os valores de limites padrão. Isso impede que todos os relatórios existentes.

Redefinir o pacote advisor pode ser feito usando o **configurar pacotes Advisor** formulário. Você precisa selecionar o pacote de Supervisor redefinido e, em seguida, clique em **redefinir**.

### <a name="remove-advisor-packs"></a>remover pacotes de Supervisor

Quando um pacote advisor não é mais necessária, os usuários poderá ser removido do banco de dados. Removendo o pacote advisor remove tudo sobre o pacote de Supervisor do banco de dados, incluindo as regras e os limites, todos os scripts SQL e todos os relatórios. Nenhuma das ações pode ser revertida.

Removendo o pacote advisor pode ser feito por meio **configurar pacotes Advisor** formulário. Você precisa selecionar o pacote advisor a ser removido e, em seguida, clique em **desprovisionamento**.
...
Se houver somente alterações de lógica de negócios e nenhuma alteração de metadados de relatório para um pacote de Supervisor, ele deve receber um novo número de versão, por exemplo do 1.0 para 2.0. Se não houver alteração de metadados do relatório, o pacote de Supervisor deve receber um nome completo diferente. Por exemplo, Microsoft.ServerPerformanceAdvisor.IIS.V1 pode ser alterado para Microsoft.ServerPerformanceAdvisor.IIS.V2.

Se uma versão mais recente do pacote de Supervisor existir, a lista nos **configurar pacotes Advisor** formulário preenche automaticamente a **versão** coluna com a versão mais recente do pacote de Supervisor. Você pode selecionar o pacote de Supervisor e, em seguida, clique no **redefinir**. As atualizações de pacote advisor com a nova lógica de negócios e limites. Todos os relatórios para este pacote advisor são preservados.
...
**Para adicionar ou remover servidores ou modificar as informações de servidor**
...
3.  Insira outras informações do servidor na **comentário** coluna para cada servidor.

    **Observação** este campo usa um formato de texto livre, portanto, você pode usá-lo como um campo de descrição. Ou usar este campo para marcar os servidores para que eles possam ser encontrados facilmente na janela principal, ou para servidores do grupo, por exemplo, por local ou função de servidor.

     

4.  Se você quiser usar SPA com um grande número de servidores, o SPA oferece suporte a um formato de valores separados por vírgula (. csv) para importação. O arquivo deve conter pelo menos dois campos: **Servidor** e **local de compartilhamento de arquivo**. O terceiro campo, **comentário** é opcional, mas é recomendável para organizar seus servidores. Você também pode exportar a lista de servidores para um arquivo. csv para determinar o formato apropriado ou fazer backup da configuração do servidor.

### <a name="searching-and-filtering"></a>Pesquisa e filtragem

Se você gerencia mais de alguns servidores, SPA fornece suporte básico para localizar rapidamente os servidores na janela principal. Você pode clicar na coluna cabeçalho para classificar com base no nome do servidor, resultados da análise, status da tarefa atual ou comentários. Você também pode optar por usar a funcionalidade de pesquisa. No canto superior direito da janela principal, você pode digitar uma cadeia de caracteres para pesquisar. O **servidor de destino** lista na janela principal usa a cadeia de caracteres para filtrar os servidores e para exibir somente os servidores com campos de nome ou comentário que contém a cadeia de caracteres de pesquisa.

A figura a seguir mostra como a cadeia de caracteres **delL** corresponde a servidores com a cadeia de caracteres **delL** ou servidores com **comentário** campo que contém **delL**.
...
1.  Em um prompt de comando do Windows PowerShell com privilégios elevados, digite **Registerspacmdlets**. O **registrar com êxito o SPA cmdlets** mensagem será exibida.

2.  Executar **SPA PowerShell.cmd**. Se você passar o caminho para um arquivo de script do Windows PowerShell, ele executar os scripts automaticamente. Caso contrário, ele abre um prompt de comando do Windows PowerShell, está pronto para executar os cmdlets do Windows PowerShell do SPA.

A tabela a seguir descreve os cmdlets do Windows PowerShell do SPA:

| Nome do cmdlet | Parâmetros | Descrição |
| ------ | ------- | ------ |
| Início SpaAnalysis | **-ServerName** nome do servidor de destino.<br>**-AdvisorPackName** nome completo do pacote do Supervisor para ser enfileirados no servidor. Quando mais de um pacote é agendado para execução ao mesmo tempo, o valor do parâmetro deve ser formatado como AP1name, AP2name.<br>**-Duração** duração para a coleta de dados.<br>**-Credencial** credenciais do usuário para a conta que executa a coleta de dados no servidor de destino.<br>**-SqlInstanceName** nome da instância do SQL Server.<br>**-SqlDatabaseName** nome do banco de dados de projeto SPA. | Inicia uma sessão de coleta de dados do SPA no servidor especificado. |
| Stop-SpaAnalysis | **-SqlInstanceName** nome da instância do SQL Server.<br>**-SqlDatabaseName** nome do banco de dados de projeto SPA.<br>**-ServerName** nome do servidor de destino. | Tenta interromper uma sessão em execução do SPA. Se uma sessão já foi concluída, ela retornará sem fazer nada. |
| Get-SpaServer | **-SqlInstanceName** nome da instância do SQL Server.<br>**-SqlDatabaseName** nome do banco de dados de projeto SPA. | Obtém a lista de servidores no banco de dados. Retorna uma lista de objetos, inclusive estas propriedades: Nome, Status, compartilhamento de arquivos e comentário. |
| Get-SpaAdvisorPacks | **-SqlInstanceName** nome da instância do SQL Server<br>**-SqlDatabaseName** nome do banco de dados de projeto do SPA | Obtém a lista de pacotes de Supervisor no banco de dados. Retorna uma lista de objetos, inclusive estas propriedades: Nome, DisplayName, autor e versão. |

O Windows PowerShell oferece a capacidade de passar credenciais por meio de arquivos criptografados para habilitar cenários de automação. Para obter mais informações sobre como usar arquivos criptografados para passar as credenciais para um cmdlet, consulte [criar Scripts do Windows PowerShell que aceitar credenciais](https://technet.microsoft.com/magazine/ff714574.aspx).
...
Por exemplo, o comando T-SQL a seguir fornece uma lista de 10 primeiros dos servidores por CPU para o período de tempo que abrangem os últimos três dias:
...
* **Crítico** registra apenas falhas e exceções que não podem ser tratadas pelo console do SPA. Essas falhas fazer com que o console do SPA falhar. Os logs fornecem as informações de contexto para tais falhas.

Por padrão, o nível de log é um aviso, o que significa que o SPA registra apenas falhas e exceções que ocorrem no SPA. O nível de log pode ser alterado editando o **SpaConsole.exe.config** arquivo na mesma pasta SpaConsole.exe está localizado. Todos os logs são gravados para o arquivo de log. txt na mesma pasta.
...
Para obter mais informações sobre os recursos de depuração do SPA, consulte o [Server Performance Advisor Pack Development Guide](server-performance-advisor-pack-development-guide.md).
...
Quando você tenta executar SPAConsole.exe pela primeira vez, se o .NET Framework não estiver instalado, o aplicativo não inicie ou gravar um arquivo de log. Certifique-se de que uma compatible.NET que Framework está instalado e funcionando corretamente antes de iniciar o SPA.

### <a name="locating-log-information"></a>Localizando informações de log

SPA armazena informações de falha no arquivo de log. txt na pasta SPA. mensagens de erro detalhadas e informações de pilha de chamadas são gravados nesta pasta. Se houver falhas que precisam de mais informações para interpretar, você pode abrir o arquivo de log. txt para ver os detalhes do erro.
...
Se você estiver usando o SQL Server Express, o banco de dados do usuário é limitado a 10 GB, mas o arquivo de log correspondente pode exceder 70 GB. Por esses motivos, é recomendável 100 GB ou mais de espaço livre em disco para o SQL Server Express. Esse espaço em disco deve ser suficiente para armazenar cerca de 20.000 para 30.000 relatórios. Esse arquivo de log é denominado SPADB\_log e ele é encontrado no **% Program Files %\\Microsoft SQL Server\\MSSQL10. SQLEXPRESS\\MSSQL\\dados**.
...
Se você receber um não é possível criar o conjunto de coleta de dados do CCP na mensagem do servidor de destino, certifique-se de fazer o seguinte:
...
### <a href="" id="running-spa-against-the-console-"></a>Executando o SPA contra o console

CCP passa por um canal diferente se o servidor de destino é o mesmo que o console do SPA. Mesmo se a conta de usuário está executando o console do SPA com privilégios de administrador, CCP falhará. Se o console do SPA é instalado em um servidor de destino, os usuários precisam iniciar SPA como um administrador para verificar se que pode executar a tarefa de análise de desempenho no console.
...
SPA protege a integridade de cada relatório que é gerado com êxito pelo SPA. ao mesmo tempo, o SPA não garante que todas as tarefas de análise na fila sejam concluídas. Se você estiver vendo inconsistente alterações de status de sessões de análise de desempenho ou erros que o sistema de declaração não é possível localizar o conjunto de logs de desempenho gerados pelo coletor de dados, é provável que seja causado por várias instâncias do console SPA em execução no mesmo banco de dados de projeto SPA.
...
Quando você executa o SPAConsole.exe e você usar uma coleção de dados recorrente (por exemplo, uma coleção por hora), o servidor que executa o SPAConsole.exe não deve estar no modo de economia de energia, de modo que ele pode suspender. SPA não verifica um política de economia de energia. Essa atividade suspensa pode interromper a coleta de dados recorrente regular.

### <a name="lost-etw-events"></a>Eventos ETW de perdidos

Para criar o impacto de desempenho mínimo nos servidores de destino, CCP foi projetado para ser executado com baixa prioridade, enquanto ele está coletando informações de desempenho. Se o servidor de destino estiver ocupado, CCP pode remover algumas das tarefas de coleta de dados para produzir a tarefas de alta prioridade que estão em execução nos servidores de destino. Você deve considerar como configurar a pasta compartilhada onde os eventos são gravados em um disco conflitantes t com a carga de trabalho de s e/s ou uma unidade mais rápida, como uma unidade SSD. Quando os eventos são removidos, são relatados na exibição do relatório como eventos perdidos podem afetar a confiabilidade das métricas de desempenho gerados.

Alguns problemas de permissão também pode causar o registro ou consultas WMI deve ser ignorada. No entanto, isso é muito menos provável de acontecer de perda de eventos ETW. Como resultado, os resultados de conjunto de Coletores de dados, às vezes, não contêm todos os valores que são solicitados. Você precisa certificar-se de que a situação é tratada pelos scripts T-SQL para todos os pacotes de Supervisor. Se os dados não existir no resultado da coleta de dados, ele é marcado como nenhum dado nos relatórios.
...
Se os resultados do coletor de dados contiverem dados incompletos diferente de perda de rastreamento do ETW e o desenvolvedor do pacote advisor tivesse adicionado suporte para notificação de perda de eventos ETW, uma barra de informações é mostrada na parte superior do relatório único para notificar o usuário sobre o potencialmente a causa perda de dados inconsistentes no relatório. informações de perda de dados detalhados podem ser encontradas no arquivo de log. txt.
