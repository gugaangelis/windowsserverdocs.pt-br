...
description: Tópico de referência para subcomando set-Server, que definiu as configurações para um servidor de serviços de implantação do Windows.
...
|[/NewMachineNamingPolicy:<Policy>]|Especifica o formato a ser usado ao gerar nomes de computador para clientes. Para obter informações sobre o formato a ser <policy>usado para o, clique com o botão direito do mouse no servidor no snap-in do MMC, clique em **Propriedades**e exiba a guia **serviços de diretório** . Por exemplo, **/NewMachineNamingPolicy:% 61Username% #**.|
...
: o subcomando[Start-Server](subcommand-start-server.md)
