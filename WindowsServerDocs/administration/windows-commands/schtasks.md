...
- **Schtasks** substitui **At.exe**, uma ferramenta incluída em versões anteriores do Windows. Embora **At.exe** ainda esteja incluído na família Windows Server 2003, o **Schtasks** é a ferramenta de agendamento de tarefa de linha de comando recomendada.
...
- Em raras ocasiões, os arquivos de tarefa ficam corrompidos. As tarefas corrompidas não são executadas. Quando você tenta executar uma operação em tarefas corrompidas, **SchTasks.exe** exibe a seguinte mensagem de erro:
  ```
...
##### <a name="u-domainuser"></a>t\<Domain>\]<User>
...
##### <a name="ru-domainuser--system"></a>/ru {[ \<Domain> \] <User> | Sistema
...
|       Valor        |                                                    Descrição                                                    |
|--------------------|-------------------------------------------------------------------------------------------------------------------|
| [\<Domain>\]<User> |                                       Especifica uma conta de usuário alternativa.                                        |
...
Fornece a senha para a conta de usuário especificada no parâmetro **/ru** . Se você omitir esse parâmetro ao especificar uma conta de usuário, **SchTasks.exe** solicitará a senha e obscurecerá o texto que você digitar.
...
|POR hora|1 - 23|A tarefa é executada a cada \<N> hora.|
...
|    MENSAIS    |          Nenhum ou {1-12}          |          1 - 31          | Opcional e válido somente sem parâmetro de modificador (**/mo**) (uma agenda de data específica) ou quando a **/mo** é 1-12 (uma agenda de cada \<N> mês). O padrão é dia 1 (o primeiro dia do mês). |
...
Especifica a hora do dia em que a tarefa é iniciada (cada vez que ela é iniciada) no \<HH:MM> formato de 24 horas. O valor padrão é a hora atual no computador local. O parâmetro **/St** é válido com minutos, por hora, diariamente, semanalmente, mensalmente e em agendas. Ele é necessário para uma única agenda.
...
| \<YYYY>/<MM>/<DD> |          Use para formatos de primeiro ano, como **Sueco** e **francês (Canadá)**.          |
...
Esse comando também usa o parâmetro **/s** para especificar o computador remoto e o parâmetro **/u** para executar o comando com as permissões da conta de administrador do usuário. Como o parâmetro **/p** é omitido, **SchTasks.exe** solicita ao usuário a senha da conta de administrador.
...
O comando a seguir agendará o programa de AdminCheck.exe para ser executado no computador público toda sexta-feira às 4:00 A.M., mas somente se o administrador do computador estiver conectado.
...
2. Use **Schtasks.exe** para criar uma tarefa que executa MyApps.bat.
   ...
   O comando a seguir cria a tarefa monitor, que é executada sempre que alguém faz logon. Ele usa o parâmetro **/TN** para nomear a tarefa e o parâmetro **/TR** a ser executado MyApps.bat. Ele usa o parâmetro **/SC** para indicar o tipo de agendamento onloginn e o parâmetro **/ru** para executar a tarefa com as permissões da conta de administrador do usuário.
   ```
...
Observe que, ao agendar tarefas em um computador remoto, todos os parâmetros se referem ao computador remoto. Portanto, o arquivo executável especificado pelo parâmetro **/TR** refere-se à cópia de MyApp.exe no computador remoto.
```
...
-   Você pode testar uma tarefa assim que criá-la. Use a operação **executar** para testar a tarefa e, em seguida, verifique se há erros no arquivo de SchedLgU.txt (*raiz_do_sistema*\SchedLgU.txt).
...
|  t\<Domain>\]<User>  |                                                                                                                                                                 Executa este comando com as permissões da conta de usuário especificada. O padrão é as permissões do usuário atual do computador local. A conta de usuário especificada deve ser um membro do grupo Administradores no computador remoto. Os parâmetros **/u** e **/p** são válidos somente para alterar uma tarefa em um computador remoto (**/s**).                                                                                                                                                                  |
|     /p \<Password>      |                                                                                                                                                                                              Especifica a senha da conta de usuário especificada no parâmetro **/u** . Se você usar o parâmetro **/u** , mas omitir o parâmetro **/p** ou o argumento password, **Schtasks** solicitará uma senha.</br>Os parâmetros **/u** e **/p** são válidos somente quando você usa **/s**.                                                                                                                                                                                               |
...
|     /ri \<Interval>     |                                                                                                                                                                                                                                                                           Especifica o intervalo de repetição para a tarefa agendada, em minutos. O intervalo válido é 1-599940 (599940 minutos = 9999 horas).                                                                                                                                                                                                                                                                            |
...
|     /du \<Duration>     |                                                                                                                                                                                                                                                                                                     Especifica para fechar a tarefa no \<EndTime> ou <Duration> , se especificado.                                                                                                                                                                                                                                                                                                      |
...
```
Como resultado desse comando, a tarefa de verificação de vírus agora é executada VirusCheck2.exe.
...
O comando a seguir altera o programa que uma tarefa executa e altera a conta de usuário na qual a tarefa é executada. Basicamente, ele usa uma agenda antiga para uma nova tarefa. Esse comando altera a tarefa ChkNews, que inicia Notepad.exe todas as manhãs às 9:00 A.M., para iniciar o Internet Explorer em vez disso.
...
O parâmetro **/ru**e **/RP** , que fornece a senha para a conta de usuário, é omitido. Você deve fornecer uma senha para a conta, mas você pode usar o parâmetro **/ru**e **/RP** e digitar a senha em texto não criptografado ou aguardar a **SchTasks.exe** para solicitar uma senha e, em seguida, digitar a senha no texto obscuro.
```
schtasks /change /tn ChkNews /tr c:\program files\Internet Explorer\iexplore.exe /ru DomainX\Admin01
```
Em resposta, **SchTasks.exe** solicita a senha para a conta de usuário. Ele obscurece o texto que você digita, portanto, a senha não é visível.
```
...
```
Como as tarefas executadas com permissões de conta do sistema não exigem uma senha, **SchTasks.exe** não solicita uma.
...
|         Termo          |                                                                                                                                                                 Definição                                                                                                                                                                  |
|-----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    /TN \<TaskName>    |                                                                                                                                                       Obrigatórios. Identifica a tarefa.                                                                                                                                                        |
|    /s \<Computer>     |                                                                                                           Especifica o nome ou o endereço IP de um computador remoto (com ou sem barras invertidas). O padrão é o computador local.                                                                                                           |
| t\<Domain>\]<User> | Executa este comando com as permissões da conta de usuário especificada. Por padrão, o comando é executado com as permissões do usuário atual do computador local.</br>A conta de usuário especificada deve ser um membro do grupo Administradores no computador remoto. Os parâmetros **/u** e **/p** são válidos somente quando você usa **/s**. |
|    /p \<Password>     |                          Especifica a senha da conta de usuário especificada no parâmetro **/u** . Se você usar o parâmetro **/u** , mas omitir o parâmetro **/p** ou o argumento password, **Schtasks** solicitará uma senha.</br>Os parâmetros **/u** e **/p** são válidos somente quando você usa **/s**.                           |
|          /?           |                                                                                                                                                    Exibe a ajuda no prompt de comando.                                                                                                                                                     |
...
-   Para executar uma tarefa remotamente, a tarefa deve ser agendada no computador remoto. Quando você o executa, a tarefa é executada somente no computador remoto. Para verificar se uma tarefa está em execução em um computador remoto, use o Gerenciador de tarefas ou o Agendador de Tarefas log de transações \<Systemroot>\SchedLgU.txt.
...
|         Termo          |                                                                                                                                                               Definição                                                                                                                                                                |
|-----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    /TN \<TaskName>    |                                                                                                                                         Obrigatórios. Identifica a tarefa que iniciou o programa.                                                                                                                                         |
|    /s \<Computer>     |                                                                                                                        Especifica o nome ou o endereço IP de um computador remoto. O padrão é o computador local.                                                                                                                        |
| t\<Domain>\]<User> | Executa este comando com as permissões da conta de usuário especificada. Por padrão, o comando é executado com as permissões do usuário atual do computador local. A conta de usuário especificada deve ser um membro do grupo Administradores no computador remoto. Os parâmetros **/u** e **/p** são válidos somente quando você usa **/s**. |
|    /p \<Password>     |                        Especifica a senha da conta de usuário especificada no parâmetro **/u** . Se você usar o parâmetro **/u** , mas omitir o parâmetro **/p** ou o argumento password, **Schtasks** solicitará uma senha.</br>Os parâmetros **/u** e **/p** são válidos somente quando você usa **/s**.                         |
|          /?           |                                                                                                                                                             Exibe a ajuda.                                                                                                                                                              |
...
O comando a seguir interrompe a instância de Notepad.exe que foi iniciada pela tarefa meu bloco de notas:
```
schtasks /end /tn My Notepad
```
Em resposta, **SchTasks.exe** interrompe a instância do Notepad.exe que a tarefa iniciou e exibe a seguinte mensagem de êxito:
```
...
```
Em resposta, **SchTasks.exe** interrompe a instância do Internet Explorer que a tarefa iniciou e exibe a seguinte mensagem de êxito:
```
...
```
Em resposta, **SchTasks.exe** exibe as seguintes mensagens de êxito indicando que a única tarefa agendada, SecureScript, é excluída.
...
|      /Fo \<format>    |  Especifica o formato de saída. Os valores válidos são tabela, lista e CSV                                                                                                                                 |
...
|    /s \<Computer>     |                                                                                                           Especifica o nome ou o endereço IP de um computador remoto (com ou sem barras invertidas). O padrão é o computador local.                                                                                                           |
| t\<Domain>\]<User> | Executa este comando com as permissões da conta de usuário especificada. Por padrão, o comando é executado com as permissões do usuário atual do computador local.</br>A conta de usuário especificada deve ser um membro do grupo Administradores no computador remoto. Os parâmetros **/u** e **/p** são válidos somente quando você usa **/s**. |
|    /p \<Password>     |                                        Especifica a senha da conta de usuário especificada no parâmetro **/u** . Se você usar **/u**, mas omitir **/p** ou o argumento password, **Schtasks** solicitará uma senha.</br>Os parâmetros **/u** e **/p** são válidos somente quando você usa **/s**.                                         |
|          /?           |                                                                                                                                                    Exibe a ajuda no prompt de comando.                                                                                                                                                     |
...
schtasks /query
```
Em resposta, **SchTasks.exe** exibe as tarefas no formato de tabela simples padrão, conforme mostrado na tabela a seguir:
```
...
Em resposta, **SchTasks.exe** exibe uma lista detalhada de propriedades para todas as tarefas. A exibição a seguir mostra a lista de tarefas para uma tarefa agendada para execução às 4:00. na última sexta-feira de cada mês:
```
HostName: RESKIT01
...
```
Em resposta, **SchTasks.exe** adiciona as tarefas agendadas para o computador Reskit16 ao arquivo de p0102.csv no computador local, SVR01.
