---
title: em
description: Tópico de comandos do Windows para **em**, que agenda comandos e programas a serem executados em um computador em uma data e hora especificadas.
...
| `every:` | Executa o *comando* em todos os dias especificados ou dias da semana ou mês (por exemplo, todas as quintas-feiras ou o terceiro dia de cada mês). |
...
| `next:` | Executa o *comando* na próxima ocorrência do dia (por exemplo, próxima quinta-feira). |
...
- **Schtasks** é outra ferramenta de agendamento de linha de comando que você pode usar para criar e gerenciar tarefas agendadas. Para obter mais informações sobre **Schtasks**, consulte [Schtasks](schtasks.md).

- Para usar **em**, você deve ser um membro do grupo local de administradores.

- **no** não carrega automaticamente cmd. exe, o interpretador de comando, antes de executar comandos. Se você não estiver executando um arquivo executável (. exe), deverá carregar explicitamente cmd. exe no início do comando da seguinte maneira:
