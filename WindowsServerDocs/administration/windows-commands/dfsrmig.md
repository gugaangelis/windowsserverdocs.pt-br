---
title: dfsrmig
description: O tópico de comandos do Windows para DFSRMIG, que migra a replicação de SYSvol do serviço de replicação de arquivo (FRS) para replicação Sistema de Arquivos Distribuído (DFS), fornece informações sobre o progresso da migração e modifica os objetos do AD DS (serviços de domínio Active Directory) para dar suporte à migração.
