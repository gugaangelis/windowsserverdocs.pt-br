...
|                             Tarefa                              |                                                                                                                                                                                                                 Comando                                                                                                                                                                                                                 |
|---------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|             Definir a senha administrativa local             |                                                                                                                                                                                                      \* de **administrador de usuário net**                                                                                                                                                                                                      |
...
|         Adicionar um usuário ao grupo local de administradores          |                                                                                                                                                                                       **Administradores do net localgroup/Add \<domínio\\nome de usuário\>**                                                                                                                                                                                       |
...
|                 Ativar o servidor remotamente                  |                                            **cscript slmgr. vbs – ipk \<chave do produto\>\<nome do servidor\>\<nome de usuário\>\<senha\>** <br>**cscript slmgr. vbs-ato \<ServerName\> \<nome de usuário\> \<senha\>** <br>Obtenha o GUID do computador executando **cscript slmgr. vbs-did** <br> Execute **cscript slmgr. vbs-dli \<GUID\>** <br>Verifique se o status da licença está definido como **licenciado (ativado)** .                                             |
...
|                         Remover uma atualização                          |                                 **Expand/f:\* \<Update\>. msu c:\test** <br>Navegue até c:\test\ e abra \<Update\>. xml em um editor de texto.<br>Substitua **instalar** por **remover** e salve o arquivo.<br>**pkgmgr/n:\<atualizar\>. xml**                                 |
...
|                         Iniciar um serviço                          |                                                                                                                                                                                 **SC start \<nome do serviço\>** ou **net start \<nome do serviço\>**                                                                                                                                                                                  |
|                          Parar um serviço                          |                                                                                                                                                                                  **sc stop \<nome do serviço\>** ou **net stop \<nome do serviço\>**                                                                                                                                                                                   |
| Recuperar uma lista de aplicativos em execução e processos associados |                                                                                                                                                                                                           **tasklist**                                                                                                                                                                                                           |
|                        Iniciar o Gerenciador de Tarefas                        |                                                                                                                                                                                                           **taskmgr**                                                                                                                                                                                                            |
|    Criar e gerenciar logs de desempenho e sessão de rastreamento de eventos    | Para criar um contador, rastreamento, coleta de dados de configuração ou API: **logman criar** <br>Para consultar as propriedades do coletor de dados: **consulta logman** <br>Para iniciar ou parar a coleta de dados: **logman iniciar\|parar** <br>Para excluir um coletor: **logman Delete** <br> Para atualizar as propriedades de um coletor: **atualização de logman** <br>Para importar um conjunto de coletores de dados de um arquivo XML ou exportá-lo para um arquivo XML: **logman import\|Export** |
...
|Eventos de consulta em um log especificado|**wevtutil qe/f: nome do log de \<de texto\>**| 
|Exportar um log de eventos|**wevtutil EPL \<nome do log\>**| 
|Limpar um log de eventos|**nome do log de \<de wevtutil do CL\>**| 
...
| Converter um volume no sistema de arquivos NTFS |        **converter \<letra do volume\>/FS: NTFS**         |
