...
-   **[Get-SilData](https://technet.microsoft.com/library/dn283388.aspx)** : exibe uma coleção pontual de todos os dados de log de inventário de software.  
...
-   **[Get-SilUalAccess](https://technet.microsoft.com/library/dn283389.aspx)** : exibe o número total de solicitações de dispositivo de cliente exclusivas e solicitações de usuário cliente do servidor de dois dias antes.  
...
> [!IMPORTANT]  
> Se for necessária uma instalação de reparo ou atualização do sistema operacional por algum motivo, os arquivos de log armazenados localmente serão perdidos.  Se esses dados forem críticos para operações, será recomendável fazer backup antes da nova instalação do sistema operacional. Após o reparo ou atualização, simplesmente restaure no mesmo local.  
...
Ao usar o log de inventário de software em um host Hyper-V do Windows Server 2012 R2, é possível recuperar dados do SIL de convidados do Windows Server 2012 R2 que estão em execução localmente, se o log do SIL tiver sido iniciado nos convidados. No entanto, isso só é possível ao usar os cmdlets do PowerShell Get-SilData e Publish-SilData e só é possível com o WIndows Server 2012 R2 no host e no convidado.  A finalidade desse recurso é permitir que data center administradores que fornecem VMs convidadas a locatários ou outras entidades de uma grande corporação capturem dados de inventário de software no host do hipervisor e, posteriormente, encaminhe todos esses dados para um agregador (ou URI de destino).  
  
Veja a seguir dois exemplos de como a saída no console do PowerShell ficaria (muito abreviada) em um host do Windows Server 2012 R2 Hyper-V executando uma VM de convidado do Windows Server 2012 R2 com o log do SIL iniciado.  Você observará que o primeiro exemplo, que usa Get-SilData sozinho, produzirá todos os dados do host conforme o esperado.  Também estão incluídos todos os dados de SIL do convidado, mas em um formato recolhido.  Para expandir e exibir esses dados do convidado, basta recortar e colar o trecho usado no segundo exemplo abaixo.  Os objetos de dados SIL do convidado sempre terão o GUID da VM associado no objeto.  
  
> [!NOTE]  
> Como os dados de SIL são emitidos no console, ao usar o cmdlet Get-SilData, em fluxos de dados, os objetos nem sempre serão emitidos em uma ordem previsível.  Nos dois exemplos abaixo, o texto foi codificado por cores (azul para dados de host físico e verde para dados de convidado virtual) somente como uma ferramenta ilustrativa para este documento.  
