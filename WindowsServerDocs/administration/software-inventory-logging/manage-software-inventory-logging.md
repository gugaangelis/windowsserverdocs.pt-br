...
Este documento descreve como gerenciar o log de inventário com a Software, um recurso que ajuda os administradores do datacenter a registrar facilmente dados de gerenciamento de ativos de software Microsoft para suas implantações ao longo do tempo. Este documento descreve como gerenciar o Log de Inventário de Software. Antes de usar o log de inventário de Software com o Windows Server 2012 R2, certifique-se de que o Windows Update [KB 3000850](https://support.microsoft.com/kb/3000850) e [KB 3060681](https://support.microsoft.com/kb/3060681) são instalados em cada sistema que precisa ser inventariado. Não há atualizações do Windows são necessárias para o Windows Server 2016. Esse recurso é executado localmente em cada servidor a ser inventariado. Ele não coleta dados de servidores remotos.  

O recurso de log de inventário de Software também pode ser adicionado a duas versões do Windows Server anteriores ao Windows Server 2012 R2. Você pode instalar as atualizações a seguir para adicionar a funcionalidade do log de inventário de Software para o Windows Server 2012 e Windows Server 2008 R2 SP1:

- **Windows Server 2012 (Standard ou Datacenter Edition)** 

> [!NOTE] 
> Verifique se você tem [WMF 4.0](https://www.microsoft.com/en-us/download/details.aspx?id=40855) instalado antes de aplicar o pacote de atualização abaixo.
...
-   [Iniciando e parando o Software de log de inventário](manage-software-inventory-logging.md#BKMK_Step1)  
...
-   [Backup e restaurando dados registrados pelo log de inventário de Software] gerenciar-software – inventário-logging.md #BKMK_Step5)  
...
-   [Trabalhando com as configurações de data e hora no log de inventário de Software do Windows Server](manage-software-inventory-logging.md#BKMK_Step8)  
  
-   [Habilitando e configurando o inventário de Software, registro em log em um disco rígido Virtual montado](manage-software-inventory-logging.md#BKMK_Step10)  
  
-   [Visão geral do uso de inventário de Software, registro em log no Windows Server 2012 R2 sem KB 3000850](manage-software-inventory-logging.md#BKMK_Step11)  
  
-   [Usando o log de inventário de Software em um ambiente do Windows Server 2012 R2 Hyper-V sem KB 3000850](manage-software-inventory-logging.md#BKMK_Step12)  
  
> [!NOTE]  
> Este tópico inclui cmdlets do Windows PowerShell de exemplo que podem ser usados para automatizar alguns dos procedimentos descritos. Para obter mais informações, consulte usando os Cmdlets.

  
## <a name="BKMK_Step1"></a>Iniciando e parando o Software de log de inventário  
Coleção de diário de log de inventário de software e encaminhamento pela rede devem ser habilitados em um computador executando o Windows Server 2012 R2 para o inventário de software do log.  
...
## <a name="BKMK_Step4"></a>Excluindo dados registrados pelo log de inventário de Software  
O Log de Inventário de Software não tem a finalidade de ser um componente crítico. Seu design destina-se a afetar as operações do sistema local o mínimo possível, mantendo um alto nível de confiabilidade. Isso também permite que o administrador exclua manualmente o banco de dados de log de inventário de Software e os arquivos (todos os arquivos no diretório \Windows\System32\LogFiles\SIL) de suporte para atender às necessidades operacionais.  
...
> [!NOTE]  
> Se por algum motivo gerenciar a retenção de duração dos dados registrados localmente pelo SIL se torna importante, isso pode ser configurado alterando o valor do registro: \HKEY_LOCAL_MACHINE\\SOFTWARE\Microsoft\Windows\SoftwareInventoryLogging. O padrão é ‘30’ para 30 dias.  
  
## <a name="BKMK_Step6"></a>Lendo dados registrados e publicados pelo log de inventário de Software  
Os dados registrados pelo SIL, mas armazenados localmente (se o encaminhamento para o URI de destino falhar), ou seja encaminhados com êxito para o servidor de agregação de destino, são armazenados em um arquivo binário (para os dados de cada dia). Para exibir esses dados no PowerShell, use o cmdlet [Import-BinaryMiLog](https://technet.microsoft.com/library/dn262592.aspx) .  
  
## <a name="BKMK_Step7"></a>Segurança do log de inventário de software  
...
## <a name="BKMK_Step8"></a>Trabalhando com as configurações de data e hora no Windows Server 2012 R2 Software log de inventário  
...
Para dar suporte a esses usos, o Log de Inventário de Software tem entradas de Registro associadas a cada opção configurável.  Esses valores do Registro podem ser encontrados em \HKEY_LOCAL_MACHINE\\SOFTWARE\Microsoft\Windows\SoftwareInventoryLogging.  
...
## <a name="BKMK_Step11"></a>Visão geral do uso de inventário de Software, registro em log no Windows Server 2012 R2 sem KB 3000850  
...
## <a name="BKMK_Step12"></a>Usando o log de inventário de Software em um ambiente do Windows Server 2012 R2 Hyper-V sem KB 3000850  
  
> [!NOTE]  
> Esta funcionalidade é removida com a instalação da atualização [KB 3000850](https://support.microsoft.com/kb/3000850) .  
  
Ao usar o log de inventário de Software em um host do Windows Server 2012 R2 Hyper-V, é possível recuperar os dados de SIL de convidados do Windows Server 2012 R2 em execução localmente, se o log de SIL tiver sido iniciado nos convidados. No entanto, isso só é possível ao usar os cmdlets Get-SilData e Publish-SilData Powershell e só é possível com o WIndows Server 2012 R2 no host e convidado.  O objetivo dessa funcionalidade é permitir que os administradores do data center que fornecem VMs convidadas para locatários ou outras entidades de uma grande corporação capturem dados de inventário de software no host do hipervisor e subsequentemente encaminhem todos esses dados para um agregador (ou URI de destino).  
  
Abaixo estão dois exemplos de VM com o log de SIL do qual a saída, o PowerShell console teria a seguinte aparência (muito abreviada) em um host do Windows Server 2012 R2 Hyper-V executando um convidado do Windows Server 2012 R2 iniciada.  Você observará que o primeiro exemplo, que usa apenas o Get-SilData, retornará todos os dados do host conforme o esperado.  Também estão incluídos todos os dados de SIL do convidado, mas em um formato recolhido.  Para expandir e exibir esses dados do convidado, basta recortar e colar o trecho usado no segundo exemplo abaixo.  Objetos de dados SIL do convidado será sempre tem o GUID da VM associada dentro do objeto.  
