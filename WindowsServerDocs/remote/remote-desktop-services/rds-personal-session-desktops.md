...
-CollectionName \<cadeia de caracteres\>
...
- **–ConnectionBroker** – especifica o servidor do Agente de Conexão de Área de Trabalho Remota para a implantação da Área de Trabalho Remota. Se você não fornecer um valor, o cmdlet usa o nome de domínio totalmente qualificado (FQDN) do computador local.
- **–User** – especifica a conta de usuário a associar com a área de trabalho de sessão pessoal no formato DOMÍNIO\Usuário. Este parâmetro é necessário.
- **–Name** – especifica o nome do servidor de host da sessão. Este parâmetro é necessário. O host de sessão identificado aqui deve ser membro da coleção que o parâmetro **-CollectionName** especifica.
...
## <a name="removing-a-user-assignment-from-a-personal-session-host"></a>Como remover uma atribuição de usuário de um host de sessão pessoal
