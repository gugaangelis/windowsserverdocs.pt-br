...
|**Configuration**|Essa configuração só terá efeito se você executar scripts.<br /><br /> Se você especificar um script de pré-atualização ou um script de pós-atualização, mas não especificar um **ConfigurationName**, a configuração de sessão padrão para o PowerShell (Microsoft. PowerShell) será usada.|Especifica a configuração de sessão do PowerShell que define a sessão na qual os scripts (especificados por **PreUpdateScript** e **PostUpdateScript**) são executados e podem limitar os comandos que podem ser executados.|  
...
##  <a name="BKMK_profile"></a>Usar perfis de execução de atualização  
