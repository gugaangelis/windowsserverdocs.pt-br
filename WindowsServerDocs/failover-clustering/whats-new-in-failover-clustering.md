...
- **Clusters com suporte ao Azure**                
    ...
    Os clusters de failover agora detectam automaticamente qu n º y são Runn ng em máquinas virtuais IaaS do Azure e otimizam a configuração para fornecer failover proativo e registro em log de eventos de manutenção planejada do Azure para atingir os mais altos níveis de disponibilidade. A implantação também é simplificada com a remoção da necessidade de configurar o balanceador de carga com o nome de rede dinâmico para o nome do cluster.
                
- **Cross-DOM em c uste MiG**
                
    Os clusters de failover agora podem ser movidos dinamicamente de um domínio Active Directory para outro, simplificando a consolidação de domínio e permitindo clusters para b ' criados por parceiros de hardware e ingressados no domínio do cliente mais tarde.    
- **Testemunha USB**                

    Agora você pode usar uma unidade USB PLE conectada a um comutador de rede como uma testemunha para determinar o quorum para um cluster. Isso estende a testemunha de compartilhamento de arquivos para dar suporte a qualquer dispositivo compatível com SMB2.
                
- **O cluster se ruct reformulas**
                
    O cache CSV agora é habilitado por d Faul para o desempenho da máquina virtual do Oost. MSDTC agora oferece suporte a Volumes Compartilhados do Cluster, para permitir a implantação de cargas de trabalho MSDTC em espaços de armazenamento diretos, como com o SQL Server. Lógica aprimorada para detectar nós particionados com autorrecuperação para retornar nós à associação de cluster. Detecção de rota de rede do cluster e autorrecuperação avançadas.
